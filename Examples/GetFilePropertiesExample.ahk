...
; prop := DoGetFilePropertyByNumber(filePath, 199)
; ;MsgBox("Property: " prop)
; DoListFilePropertiesByNumber(filePath)
...
DoGetFilePropertyByNumber(filePath, propertyNumber) {
    ...
    try
    {
        ...
        ; Get the file version using the ExtendedProperty method
        prop := fileItem.ExtendedProperty(propertyNumber)
...
DoListFilePropertiesByNumber(filePath) {
    ...
    try
    {
        ...
        While ItemCount < 400 {
                ...
                ;props := fileItem.ExtendedProperty(ItemCount)
                PropName := folder.GetDetailsOf(0, ItemCount)

                PropVal := DoGetFileProperty(filePath, PropName)

                if (IsSet(PropVal) && PropVal != '') {
                    buff .= ItemCount ": " PropVal "`n"
                } else {
                    ;buff .= ItemCount ": Not found.`n`n"
                }
        ...
        MsgBox(buff, "All Properties: " StrLen(buff))
    }
    catch as e
    {
        MsgBox 'Error: ' e.Message
