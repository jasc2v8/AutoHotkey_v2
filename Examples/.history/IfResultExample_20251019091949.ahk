...
ChangeConsoleFont() {
    ...
    NumPut(cbSize, FontInfo, 0, "UInt") ; cbSize (size of the struct)
    ; NumPut(0, FontInfo, 4, "UInt") ; nFont (leave as 0)
    NumPut(24 << 16, FontInfo, 8, "Int64") ; dwFontSize (height << 16 | width). Use Int64 for COORD type.
    NumPut(0x36, FontInfo, 16, "UInt") ; FontFamily (0x36 is FF_DONTCARE | FF_MODERN)
    NumPut(700, FontInfo, 20, "UInt") ; FontWeight (700 = bold)

    ; FaceName is a wide string (UTF-16).
    FontName := "Consolas"
    NumPut(FontName, FontInfo, 24, "WStr")
