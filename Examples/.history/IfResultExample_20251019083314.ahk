...
ChangeConsoleFont(FontName, FontSizeX, FontSizeY)
{
    ...
    NumPut(FONT_INFO_SIZE, ConsoleFontInfo, 0, "UInt") ; cbSize (4 bytes)
    
    ; dwFontSize (X and Y coordinates, 8 bytes total)
    NumPut(FontSizeX, ConsoleFontInfo, 4*2, "Int") ; X at offset 8
    NumPut(FontSizeY, ConsoleFontInfo, 4*2 + 4, "Int") ; Y at offset 12

    ; FontFamily (Raster or Truetype) - 52 (FF_DONTCARE | FF_MODERN) is typical for TrueType
    NumPut(52, ConsoleFontInfo, 16, "UInt") ; FontFamily at offset 16 (4 bytes)

    ; FontWeight (e.g., 400 for Normal, 700 for Bold)
    NumPut(400, ConsoleFontInfo, 20, "UInt") ; FontWeight at offset 20 (4 bytes)
