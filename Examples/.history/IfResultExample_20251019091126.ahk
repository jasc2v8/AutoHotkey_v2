...
r := ChangeConsoleFont("Times New Roman", 12)
if (r=0)
...
ChangeConsoleFont(FontName, FontSizeY, FontWeight := 400)
{
    ...
    NumPut(FONT_INFO_SIZE, ConsoleFontInfo, 0, "UInt") 
    
    ; Set the font size (dwFontSize - 8 bytes)
    ; FontSizeX: 0 means Auto-select best width
    NumPut(0, ConsoleFontInfo, 4*2, "Int")         ; X at offset 8
    NumPut(FontSizeY, ConsoleFontInfo, 4*2 + 4, "Int") ; Y (Height) at offset 12

    ; Set the font family (FF_DONTCARE=0, FF_MODERN=48, 48|4=52 is modern monospace)
    NumPut(52, ConsoleFontInfo, 16, "UInt")        ; FontFamily at offset 16

    ; Set the font weight (e.g., 400 for Regular, 700 for Bold)
    NumPut(FontWeight, ConsoleFontInfo, 20, "UInt") ; FontWeight at offset 20
    ...
    StringPut(FontName, ConsoleFontInfo, 24, "UTF-16") 
...
ChangeConsoleFont_OLD(FontName, FontSizeX, FontSizeY)
{
    ; 1. Get the console output handle
    STD_OUTPUT_HANDLE := -11
    hConsoleOutput := DllCall("GetStdHandle", "int", STD_OUTPUT_HANDLE, "ptr")
