...
/*
    TODO:
        Changed to a static class because when the client exists, 
        the __Destory method deletes all the lock and Shared memory files.
*/
...
class SharedFile {
    ...
    static SharedFilePath:= A_Temp "\SharedFile.tmp"
    static SharedFileObj:= unset
    static FileObjMap:= Map()

    static CreateSharedFile(SharedFileName:="") {

        if (SharedFileName != "") {
            this.SharedFilePath := A_Temp "\" SharedFileName ".tmp"
        }
    ...
    static CreateLock(LockName:="MySharedFile", Info:="") {
    ...
    static GetLockFilename(LockName) {
    ...
    static GetSharedFilepath() {
        return this.SharedFilePath
    }

    static Lock(LockName) {
        lockFile:= this.GetLockFilename(LockName)
        if FileExist(lockFile) {
            FileSetAttrib("+R", lockFile)
    ...
    static WaitLock(LockName) {
        ;?
    }

    ; unlocked=true, timeout=false, wait forever=-1
    static WaitUnLock(LockName, TimeoutMs:=-1) {
...
SharedFile__Tests() {
    ...
    Test1() {
        ...
        if FileExist(SharedFile.SharedFilePath) {
