; ABOUT:    MyScript v0.0
; SOURCE:   Ahkv1 A_AhkUser https://www.autohotkey.com/boards/viewtopic.php?t=30038
; LICENSE:  The Unlicense, see https://unlicense.org

/*
    TODO:
*/

#Requires AutoHotkey v2.0+
#SingleInstance Force
#NoTrayIcon

LogFile:= "D:\ServiceLog.txt"

;MsgBox "I will wait until lock.txt is available"

FileAppend FormatTime(A_Now, "HH:mm:ss") "Waiting for lock.txt to be available", LogFile

WaitWrite("Message from Service.")

r := WaitRead()

;Write to file and lock it
    try {
        ; FileOpen will throw an error if the file is already being accessed
        f := FileOpen("d:\lock.txt", "a-rwd") 
        f.Write("From Service")
        f.Close()
    }

WaitWrite(Message) {

    Loop {
        try {
            ; FileOpen will throw an error if the file is already being accessed
            f := FileOpen("d:\lock.txt", "a-rwd") 
            f.Write(Message "`n")
            f.Close()
            break
        }
        Sleep 200 ; Retry every 200 milliseconds
    }

    ; 

}
Persistent

;MsgBox "Script2 now has access to lock.txt"

;FileAppend FormatTime(A_Now, "HH:mm:ss") "Waiting for lock.txt to be available", LogFile
