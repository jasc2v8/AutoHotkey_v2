...
/*
    AHKsock v2 - A simplified translation of AHKsock to AHK v2
*/

AHKsock_Listen(sPort, sFunction := False) {
    
    ; Check if there is already a socket listening on this port
    if (sktListen := AHKsock_Sockets("GetSocketFromNamePort", " ", sPort)) {
    ...
    ; Make sure we even have a function (v2 uses HasMethod or direct check)
    if !HasMethod(sFunction) && !IsFunc(sFunction) && (sFunction !== False)
    ...
    ; addrinfo struct is 16 + 4 * A_PtrSize
    aiHints := Buffer(16 + 4 * A_PtrSize, 0)
    NumPut("Int", 1, aiHints, 0)  ; ai_flags = AI_PASSIVE
    NumPut("Int", 2, aiHints, 4)  ; ai_family = AF_INET (IPv4)
    NumPut("Int", 1, aiHints, 8)  ; ai_socktype = SOCK_STREAM
    NumPut("Int", 6, aiHints, 12) ; ai_protocol = IPPROTO_TCP
    
    ; v2: sPort must be passed as a pointer to a string
    pPort := StrPtr(String(sPort))
    
    aiResult := 0
    iResult := DllCall("Ws2_32\getaddrinfo", "Ptr", 0, "Ptr", pPort, "Ptr", aiHints, "Ptr*", &aiResult, "Int")
    
    if (iResult != 0) {
        return 5 ; ErrorCode in iResult
    }
    
    ; Create a socket
    sktListen := DllCall("Ws2_32\socket", 
        "Int", NumGet(aiResult, 4, "Int"), 
        "Int", NumGet(aiResult, 8, "Int"), 
        "Int", NumGet(aiResult, 12, "Int"), "Ptr")
        
    if (sktListen = -1) { ; INVALID_SOCKET
        DllCall("Ws2_32\freeaddrinfo", "Ptr", aiResult)
        return 6
    }

    ; Bind the socket
    iResult := DllCall("Ws2_32\bind", "Ptr", sktListen, "Ptr", NumGet(aiResult, 16 + 2 * A_PtrSize, "Ptr"), "Int", NumGet(aiResult, 16, "Int"), "Int")
    DllCall("Ws2_32\freeaddrinfo", "Ptr", aiResult) ; Free memory

    if (iResult = -1) {
        return 7
    }

    ; ... Continue to WSAAsyncSelect and Listen ...
}

; Mock-up of AHKsock_Startup for v2
AHKsock_Startup() {
    static WSADATA := Buffer(A_PtrSize = 8 ? 408 : 392, 0)
    if (AHKsock_Sockets("IsStarted"))
        return 0
    
    iResult := DllCall("Ws2_32\WSAStartup", "UShort", 0x0202, "Ptr", WSADATA, "Int")
    if (iResult != 0)
        return 1
    return 0
}