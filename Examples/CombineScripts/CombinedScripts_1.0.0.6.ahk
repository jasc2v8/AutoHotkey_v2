...
/**
 * Script Merger v1.0.0.6
 * Updated: Removed AlwaysOnTop and rearranged bottom row buttons.
 */

; GUI Creation with MinSize (Removed +AlwaysOnTop)
MyGui := Gui("+Resize +MinSize420x460", "Script Merger v1.0.0.6")
...
MyGui.Add("Text", "w420 vTitleText", "Select and order the AHK v2 scripts to combine:")
FileList := MyGui.Add("ListBox", "r10 w420 vSelectedFiles")

; Top Row Buttons
BtnAdd := MyGui.Add("Button", "y+10 w70 vBtnAdd", "Add")
BtnAdd.OnEvent("Click", AddFiles)

BtnClear := MyGui.Add("Button", "x+5 yp w70 vBtnClear", "Clear")
...
BtnUp := MyGui.Add("Button", "x+5 yp w70 vBtnUp", "Up")
BtnUp.OnEvent("Click", MoveUp)

BtnDown := MyGui.Add("Button", "x+5 yp w70 vBtnDown", "Down")
...
ChkStripComments := MyGui.Add("Checkbox", "x+20 vStripComments", "Strip all comments")

; Bottom Row Buttons - Rearranged positions
BtnMerge := MyGui.Add("Button", "Default w140 xm vBtnMerge", "Merge and Save As...")
BtnMerge.OnEvent("Click", MergeScripts)

BtnCancel := MyGui.Add("Button", "x+10 w100 vBtnCancel", "Cancel")
...
Gui_Size(GuiObj, WindowMinMax, Width, Height) {
    ...
    ; Move Bottom Row - Merge | Cancel | Folder
    GuiObj["BtnMerge"].Move(, MergeY)
    GuiObj["BtnCancel"].Move(Width - 220, MergeY)
    GuiObj["BtnFolder"].Move(Width - 110, MergeY)
}
...
MergeScripts(*) {
    ...
    CombinedContent .= "; Version: 1.0.0.6`n`n"
