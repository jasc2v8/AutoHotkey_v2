...
; Run both this script and test\demo-ipc-2.ahk. Input a name or leave the default "\" and click
; "Open file mapping object". This will populate the name field with the actual name and copy the
; name to the clipboard. Paste the name to the other window and also click "Open file mapping object".
; Then try writing to one then reading from the other.
...
class Demo_FileMapping {
    static Call() {
        ...
        g.Add('Edit', 'w' width ' r' rows ' Section vEdtInput', RegExReplace('
        (
            Run both this script and test\demo-ipc-2.ahk. Input a name or leave the default "\" and click
            "Open file mapping object". This will populate the name field with the actual name and copy the
            name to the clipboard. Paste the name to the other window and also click "Open file mapping object".
            Then try writing to one then reading from the other. For example, click the "Write" button
            on this window, which will write this text to the file, then click the "Read" button
            on the other window. Then try deleting the text and writing your own message. The content
            gets read from and written to the file mapping object's current position. Use the "Set position"
