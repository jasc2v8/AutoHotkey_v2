...
Loop 10 {
    ...
    oGui.AddButton("xm Default","Font " index).OnEvent("click",button_click)
    oGui.AddEdit("yp w150 h30 -Multi vFontName" index, "Veranda")
    oGui.AddEdit("yp w500 h30 -Multi vMyEdit" index, SampleText)
}

oGui.AddText("xm w720 h2 0x10","Horizontal Line")  ;SS_ETCHEDHORZ
oGui.AddButton("xm w75 Default","MonoSpace").OnEvent("click", ButtonMonoSpace_Click)
oGui.AddButton("yp w75","Save") ; .OnEvent("click",button_click)
oGui.AddButton("yp w75","Load") ; .OnEvent("click",button_click)
oGui.AddButton("yp w75","Cancel").OnEvent("Click", (*) => ExitApp())
oGui.AddText("xm w1 h2 Hidden","Hidden Spacer")  ;SS_ETCHEDHORZ

; ctl := oGui.AddEdit("yp w500 h20 vMyEdit1", SampleText)
; oGui.AddEdit("w500 h200 vMyEdit2","Sample Text")
;oGui["Btn"].Focus()

fontObj := {name:"Courier", size:12, color:0x000000, strike:0, underline:0, italic:0, bold:0} ; init font obj (optional)
...
button_click(ctl,info) {
    ...
    ctl.gui["FontName" index].Text :=fontObj.name
    ctl.gui["MyEdit" index].SetFont(fontObj.str,fontObj.name) ; use fontObj.str for the AHK compatible string to set all selected styles
