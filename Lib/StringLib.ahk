...
class Str {
    ...
    /**
     * Joins multiple strings into a valid Windows path.
     * Handles slashes and drive colons automatically.
     */
    static JoinPath(p*) {
        r := ""
        for i, v in p {
     ...
     * @param delims String or Array of delimiters.
     * @param omit Chars to omit from the start/end of each item.
     * @param limit Maximum number of substrings to return.
     */
    static Split(str, delims := "", omit := "", limit := -1) {
        return StrSplit(str, delims, omit, limit)
    }
