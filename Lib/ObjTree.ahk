;SOURCE: https://www.autohotkey.com/boards/viewtopic.php?f=83&t=39789&hilit=modify+edit+options
	...
	TVSelect(LV, TV, item){
		...
		Loop {
			...
			LV.Add(((IsObject(v) || IsObject(k)) ? "Check" : "") (text = (IsObject(k) ? (Chr(177) " " (&k)) : k) ? (LV_CurrRow := A_Index, " Select"):"")
						, IsObject(k)?(Chr(177) " " (&k)):k, IsObject(v)?(Chr(177) " " (&v)):v, item) ;IsObject(v) && IsFunc(v)?"[" (v.IsBuiltIn?"BuildIn ":"") (v.IsVariadic?"Variadic ":"") "Func] " v.Name:
