; TITLE:    IPCSendMessageBridge v1.0
...
class IPCSendMessageBridge {
    ...
    __New(Role, ServerName, ServerKey, Callback) {

        this.ServerName := ServerName
        this.ServerKey := ServerKey

        this.Listen(Role, ServerName, ServerKey, Callback)
    }
     ...
     */
     Listen(Role, ServerName, ServerKey, Callback) {
     ...
     */
     ...
     _OnReceive(wParam, lParam, msg, hwnd) {
        ...
        ;MsgBox "IPCSendMessage receivedText:" receivedText, 'DEBUG'

        ; Check if it's a standard exit command
        if (receivedText == "IPC_EXIT") {
            MsgBox "IPCSendMessage Exit..."
            ExitApp()
        }
