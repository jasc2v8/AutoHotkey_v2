...
["URL", this.GetMenu("websites"), "chrome.ico", "!+u",], 
Note the disappearance of (*) =>> from the syntax. Instead, we use this.getmenu("websites"). If the array was programItems, we would use this.getmenu("programs").
...
Excluded scripts:
- Scripts can be excluded from the Running Scripts Menu by editing the static excludedscripts array within the Macropad class shown here. I use this to hide the Executable files and scripts I never edit on my config, but feel free to add your own here. :
    static excludedScripts := [ ; 
    "launcher.ahk",
    "AutoHotkeyUX.exe",
    "Ahk2Exe.exe", 
  ;  "MySampleAhkScript.ahk", 
    ]
