...
/**
 * TODO:
 *  fix all the margin options for more clarity
 * 
 */
...
shortMessage := unset
Loop 12
    shortMessage .= Format("{:02}", A_Index) ": The quick brown fox jumps over the lazy dog's back. The quick brown fox jumps over the lazy dog's back.`n"
shortMessage := SubStr(shortMessage, 1, StrLen(shortMessage)-1)
...
;MsgBoxEx("Press OK to continue.")

MsgBoxEx(shortMessage)
r :=MsgBoxEx(longMessage)
MsgBoxEx("You pressed: " r)
return
;ExitApp
...
;    {
      MyText := MyGui.AddText(" x" LeftMargin " y" WBMargin    " BackgroundTrans vTextBox Border", Text)   ; Add the text to the Gui
      ...
      WhiteBoxHeight := SizeY+SizeH+WBMargin                                         ; Calculate the height of the white box
;    }
;    else                                                                       ; If the text field is blank ...
;    {  GuiWidth := MinGuiWidth                                                 ; Set the width of the Gui to MinGuiWidth
;       WhiteBoxHeight := 2*WBMargin  +1                                               ; Set the height of the white box
;       WBMarginBottom    ++                                                             ; Increase the WBMargin     above the button by one
...
;    }
   ...
   ButtonY := WhiteBoxHeight+WBMarginBottom                            ; Calculate the vertical position of the button
