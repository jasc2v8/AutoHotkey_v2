...
/**
 * TODO:
 *    when gui is open, the arrow keys are backward because thats how we loaded the Gui
 * ;TODO buttonCount includes "Default", subtract 1 if contains "default
 * 
 */
...
; MsgBox Text, Title
;Buttons:="&OK, &Abort, Retry, Ignore, Yes, No, Cancel")

MsgBoxEx(Text:="Press OK to continue.", Title:=A_ScriptName, Buttons:="&OK") { ; "&OK, Cancel"
   ...
   {  MyText := MyGui.AddText(" x" LeftMargin " y" Gap " BackgroundTrans vTextBox", Text)   ; Add the text to the Gui
      ...
      ;TODO buttonCount includes "Default", subtract 1 if contains "default"
      GuiWidth := LeftMargin+SizeW+(ButtonOffset*buttonCount)+RightMargin+1-9                 ; Calculate the Gui width
   ...
   GuiCenter := GuiWidth / 2 - ButtonWidth/2 - buttonMargin

   ButtonLeftMargin := GuiCenter - (ButtonWidth * ButtonCount/2) + LeftMargin + ButtonMargin     ;GuiWidth / ButtonCount ;LeftMargin

   ;========= add buttons
	Loop buttonArray.Length { ;for item in buttonArray {

      ;index := buttonArray.Length - (1 * A_Index-1)
      index :=A_Index

      buttonText := Trim(buttonArray[index])

      ;ButtonX := GuiWidth-RightMargin-(ButtonWidth* A_Index)

      ;ButtonLeftMargin += (ButtonWidth + ButtonMargin) * A_Index-1
      
      ButtonX := ButtonLeftMargin  ; + ((ButtonWidth + ButtonMargin))

      ButtonLeftMargin += ButtonWidth + ButtonMargin
;GuiCenter - ((ButtonWidth + ButtonMargin) * ((buttonCount * .5 +.5) * A_Index-1))

;ButtonX := LeftMargin + ((ButtonWidth + ButtonMargin) * ((buttonCount * .5 +.5) * A_Index-1))
;ButtonX := LeftMargin + ((ButtonWidth + ButtonMargin) * (buttonCount - A_Index-1))
...
;    ButtonX := ButtonX-RightMargin-ButtonWidth                 ; Calculate the horizontal position of the button
;    MyGui.AddButton("x" ButtonX " y" ButtonY " w" ButtonWidth " h" ButtonHeight " Default", "Yes")
