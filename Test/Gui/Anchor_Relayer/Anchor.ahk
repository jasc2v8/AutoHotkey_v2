...
#Warn Unreachable, off
    ...
    Test1() {
        ...
        g.AddText("w580 h260 Border BackgroundDefault vText")

        ; g.AddText("xm w50 h1 0x10 vLine", "Horizontal Line")  ;SS_ETCHEDHORZ


        g.AddButton("w75 Default", "OK").OnEvent("click", Button_Click)
        g.AddButton("yp w75 Default", "Cancel").OnEvent("click", Button_Click)
		...
		;g.OnEvent("Size", Gui_OnSize_Example)
        g.Show("w600 h300")
        ;g.Show()

		Button_Click(Ctrl, Info) {

			;Anchor(Control, Anchor, Redraw)

			;Anchor(g["Line"], "x", true)
		...
		Gui_OnSize(GuiObj, MinMax, Width, Height)
		{
			...
			Anchor(GuiObj["Text"].Hwnd, 	"x0 y0 w h", redraw)
			Anchor(GuiObj["OK"].Hwnd, 		"x0 y", redraw)
			Anchor(GuiObj["Cancel"].Hwnd, 	"x0 y", redraw)
		}

		Gui_OnSize_Example(GuiObj, MinMax, Width, Height)
		{
			; GuiObj is the Gui object that fired the event (MyGui in this case).
			; MinMax indicates the state of the window:
			;   0 = neither minimized nor maximized (normal resize)
			;   1 = maximized
			;  -1 = minimized

			; If the window is minimized (-1), stop, as controls can't be resized.
			if (MinMax == -1)
				return

			; Define the margin (space between the control and the window edges)
			margin := 10

			; Get a reference to the Edit control
			Local MyEdit := GuiObj["Text"] ; Use GuiObj[Control_vName]

			; Move and resize the Edit control
			; The new width and height are the window's new dimensions (Width, Height) minus the margins.
			MyEdit.Move(margin, margin, Width - (margin * 2), Height - (margin * 2))

			; Note: The first two parameters in .Move() are x, y, and the last two are w, h.
		}
