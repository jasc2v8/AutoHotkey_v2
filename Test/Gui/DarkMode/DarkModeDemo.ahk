...
grui:= Gui2(opt:="+AlwaysOnTop", title:="Dark Mode Demo")

grui.AddListeningRadio("Checked cWhite", "DarkMode", OnRadio )

grui.AddButton("", "Cancel", OnButtonCancel)

grui.Call()

OnRadio(Ctrl, Info) {

    static isChecked := false ; Track state internally
    
    if (isChecked) {
        Ctrl.Value := 0  ; Uncheck
        isChecked := false

        SetWindowAttribute(grui, DarkMode := False)
        SetWindowTheme(grui, DarkMode := False)

    } else {
        Ctrl.Value := 1  ; Check
        isChecked := true

        SetWindowAttribute(grui, DarkMode := True)
        SetWindowTheme(grui, DarkMode := True)
