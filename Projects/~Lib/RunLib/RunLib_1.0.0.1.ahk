; TITLE  :  RunLib v1.0.0.1
; SOURCE :  jasc2v8
; LICENSE:  The Unlicense, see https://unlicense.org
; PURPOSE:  Library to run any Command line (handles spaces in the arguments) without a Command window.
; USAGE  :  runner := RunLib()
;           runner.Run(Array, CMD, CSV, Executable, or String)
...
class RunLib {
    ...
    _ParseCommand(Command, &IsExe) {
        ...
        ; If ahk then prepen with autohotkey.exe
    ...
    _ArrayToCommand(CommandArray) {
        ...
        for part in CommandArray {
            part := Trim(part)
            ; RegExMatch checks if the entire string starts and ends with a double quote.                  
