...
; EXAMPLES:
;   RunShell(Command)                         ; Determines if Array, CSV, String, Executable, or CMD.
...
;   RunShell('dir /b "D:\My Dir"'')           ; String, CMD (User must add quotes as needed.)
...
class RunShell{
    ...
    static Call(CommandLine) {
        ...
        if this.IsType(CommandLine, "CSV") {
            ...
            return this.RunWait(CommandLine)
    ...
    static RunArray(CommandArray) {
        ...
        for part in CommandArray {
            part := Trim(part)
            if InStr(part, A_Space) && !RegExMatch(part, '^".*"$') ; RegExMatch checks if the entire string starts and ends with a double quote.                  
    ...
    static RunWait(Command) {

        split := StrSplit(Command, A_Space)
        SplitPath(split[1], , , &Ext)
        IsExe := (Ext = "exe") ? true : false

        ;MsgBox "Cmd:`n`n" Command, IsExe
    ...
    static RunArray_WORKS(CommandArray) {
    ...
    static RunWait_WORKS(Command, Type:="CMD") {
