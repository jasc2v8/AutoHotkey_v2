; 2025-12-30 jasc2v8
;   Changed Peep to ObjView and Saved as ObjView
; 2025-12-26 jasc2v8
...
/**
 * @description {@link https://github.com/GroggyOtter/ObjViewAHK/|`ObjView()`}
 ...
 * - `true` = [Default] Code execution will pause when the ObjView() GUI is shown. This acts similarly to how MsgBox() works.  
 *   Code is resumed when Continue or Unpause is clicked.
 * - `false` = Code execution continues immediately after displaying the ObjView() GUI.
 ...
 * - `Positive number` = [Default] Use the built-in GUI to display ObjView() information.  
 * - `Negative number` = Use AHK's {@link https://www.autohotkey.com/docs/v2/lib/MsgBox.htm|MsgBox()} to display ObjView() information.  
 * - `0` = [Default] No information is displayed but a ObjView object is returned. The text can be retrieved by calling it.  
 * 
 *     ObjView_obj := ObjView(data)
 *     MsgBox(ObjView_obj())
 ...
 * Default: A_ScreenWidth / 2 - (ObjView.gui_w/2)
 * @property {Number} gui_y  
 * The default starting height for the GUI.  
 * Default: A_ScreenHeight / 2 - (ObjView.gui_h/2)
 * @returns {ObjView}
 * A ObjView object is always returned.  
 * The ObjView text can be retrieved by calling the returned object or accessing the `Text` property.  
 * 
 *     item := [1,2,3]
 *     pobj := ObjView(item)
 *     MsgBox(pobj())
 *     Msgbox(pobj.Text)
 * @example <caption>Multiple ObjView() examples.</caption>
 * #Include ObjView.v2.ahk
 * 
 ...
 * )
 * ObjViewobj := ObjView(item)
 * ; To view ObjView text again, call the returned object.
 * MsgBox(ObjViewobj())
 */
class ObjView {
     ...
     * If true, code execution will pause when the ObjView() GUI is shown. This acts similarly to how {@link https://www.autohotkey.com/docs/v2/lib/MsgBox.htm|MsgBox()} works.  
     * If false, code execution continues immediately after displaying the ObjView() GUI.
     * Default is: 1
     */
     ...
     * If set to a positive number, ObjView's custom GUI is used to display information.  
     * If set to a negative number, the standard MsgBox() is used.  
     * If set to zero, nothing is displayed. A ObjView object is still returned for text retrieval. 
     * Default is: 1
     */
     ...
     * Default is: A_ScreenWidth / 2 - (ObjView.gui_w/2)
     */
    static gui_x := A_ScreenWidth / 2 - (ObjView.gui_w/2)
    /** @prop {Number} gui_y  
     * This is the y value of the upper left corner.  
     * Default is: A_ScreenHeight / 2 - (ObjView.gui_h/2)
     */
    static gui_y := A_ScreenHeight / 2 - (ObjView.gui_h/2)
    ...
    static reset_to_default() {
        ...
        this.gui_x := A_ScreenWidth / 2 - (ObjView.gui_w/2)
        this.gui_y := A_ScreenHeight / 2 - (ObjView.gui_h/2)
        this.gui_h := 1000
        this.gui_w := 600
        this.gui_title := ''
    }
    ...
    __New(Title:='', items*) {
        ...
        if (items.length < 1) {
            MsgBox('No items were provided to ObjView()')
            return ''
        }
        ...
        for value in items {
            ...
            else txt .=  ObjView.unset_value1
            txt .= "`n`n"
        }
        txt := Trim(txt, '`n') '`n'
        ; Display text
        if (ObjView.display_with_gui > 0)
            this.gui_display(txt)
        else if (ObjView.display_with_gui < 0)
            MsgBox(txt)
    ...
    extract(item, ind) {
        ...
        switch {
            ...
            case (item is Primitive):
                if ObjView.include_prim_type
                    data .= '<' StrUpper(item_type) '> '
                if (item_type = 'String')
                    if (ObjView.include_string_quotes > 0)
                        data .= '"' item '"'
                    else if (ObjView.include_string_quotes < 0)
                        data .= "'" item "'"
                    else data .= item
                else data .= item
                return data
            ; Var Refs
            case (item is VarRef):
                return ObjView.var_ref_value
            ; COM values
            case ObjView.is_com_type(item_type):
            ...
            case ((item is Object) && this.is_duplicate(item)):
                return ObjView.duplicate_ref_value '<Type: ' item_type '>'
            ; The "I f*@ked up" clause
            case !IsObject(item):
                return MsgBox('GroggyOtter didn`'t account for this.'
                    '`nVisit the ObjView() GitHub page and let him know '
                    'know he missed something up. The item type is:`n' item_type)
        }
        
        ind2 := ind ObjView.indent_str
        ind3 := ind2 ObjView.indent_str
        
        ; Handle built-in props and own props
        if ObjView.include_built_in
        ...
        switch {
            ; Assemble array data
            case (item is Array):
                not_inline := (ObjView.include_built_in || !ObjView.array_values_inline || own_props != '' || !this.all_elements_are_prim(item))
                header := item_type '['
                for index, value in item {
                    if not_inline {
                        if ObjView.include_array_index
                            if ObjView.key_value_inline
                                data .= '`n' ind2 index ': '
                            else data .= '`n' ind2 index ': `n' ind3
                        else if ObjView.key_value_inline
                            data .= '`n' ind2
                        else data .= '`n' ind2
                    }
                    if IsSet(value)
                    ...
                    else data .= ObjView.unset_value
                    if ObjView.include_end_commas
                        data .= ', '
                }
                this.trim_end_comma(data)
                if not_inline
                    end_cap := '`n' ind
                        . (ObjView.key_value_inline ? '' : ind)
                        . (ObjView.indent_closing_tag ? ind2 : '')
                        . ']'
                else end_cap := ']'
            ...
            case (item is Gui.Control):
                ...
                data .= ObjView.include_built_in ? this.add_built_in(item, 'Gui.Control', ind2) : ''
            ...
            case ObjView.is_enumerable(item):
    ...
    add_built_in(item, typ, ind, is_gui_con:=0) {
        ...
        if is_gui_con
            typ := 'Gui.Control'
        for k, prop in ObjView.props[typ] {
    ...
    make_gui() {
        ...
        m           := 5
        bh          := 30
        bw          := (gw - m * (ObjView.btn_data.Length + 1)) / ObjView.btn_data.Length
        edw         := gw - m * 2
        edh         := gh - m * 3 - bh
        bg_color    := ObjView.dark_mode ? '101010' : 'F0F0F0'
        font_color  := ObjView.dark_mode ? 'D8D8D8' : '101010'
        happy       := 420
        
        ; Gui
        goo := Gui('+Resize -DPIScale +MinSize400x200', 'ObjView(' this.gui_title ')', this)
        goo.btn_h := bh
    ...
    gui_resize(goo, *) {
        ...
        bh := goo.btn_h
        by := gui_h - pad - bh
        bw := (gui_w - pad * (ObjView.btn_data.Length + 1)) / ObjView.btn_data.Length
        for control in goo
            switch control.Type {
                case 'Button':
    ...
    gui_display(txt) {
        ...
        this.gui.show('x' ObjView.gui_x ' y' ObjView.gui_y ' w' ObjView.gui_w ' h' ObjView.gui_h)
        control := this.get_default_gui_btn()
        this.gui[control].Focus()
        if ObjView.gui_pauses_code
            Pause ; pause 1 If omitted, the current thread is paused. (helps if Admin)
    }
    
    get_default_gui_btn() {
        name := ObjView.default_gui_btn
    ...
    save_to_clip(*) {
        ...
        TrayTip('Text copied to clipboard', 'ObjView(AHK)', 0x10)
    }
