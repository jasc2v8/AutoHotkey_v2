...
class SharedMemory {
    ...
    __New(Name, Size := 4096, IsServer := false) {
        ...
        this.Mapping := DllCall("CreateFileMapping", "Ptr", -1, "Ptr", SA, "UInt", SharedMemory.PAGE_READWRITE, "UInt", 0, "UInt", Size, "Str", this.Name, "Ptr")
        this.View := DllCall("MapViewOfFile", "Ptr", this.Mapping, "UInt", SharedMemory.FILE_MAP_ALL_ACCESS, "UInt", 0, "UInt", 0, "Ptr", Size, "Ptr")

        ; 3. Setup Events
        this.hEventSrv := DllCall("CreateEvent", "Ptr", SA, "Int", 0, "Int", 0, "Str", this.Name . "_Srv", "Ptr")
        this.hEventCli := DllCall("CreateEvent", "Ptr", SA, "Int", 0, "Int", 0, "Str", this.Name . "_Cli", "Ptr")
    ...
    Write(Text) {
        ...
        hEvent := this.IsServer ? this.hEventSrv : this.hEventCli
    ...
    WaitRead(Timeout := -1) {
        ...
        hEventWait := this.IsServer ? this.hEventCli : this.hEventSrv
    ...
    __Delete() {
        ...
        if (this.hEventSrv)
            DllCall("CloseHandle", "Ptr", this.hEventSrv)
        if (this.hEventCli)
            DllCall("CloseHandle", "Ptr", this.hEventCli)
    }
}