...
; SOURCE :  Gemini and jasc2v8 2025-12-28
...
class Messenger {
    ...
    _HandleIncoming(wParam, lParam, UserCallback) {
        ...
        this.SenderHWND := wParam
        ;MsgBox this.SenderHWND, "Messenager"
        ...
        if (SenderKey != this.PassKey) {
            ...
            MsgBox SenderKey " : " this.PassKey, "DEBUG"
    ...
    /**
     * Sends a reply to the window/script that sent the message.
     * @param TargetTitle The Title or the HWND of the receiving window.
     * @param DataString The text you want to send.
     * @param Timeout In Milliseconds. 0=Infinite.
     * @return True if success, else False.
     */
    Reply(DataString, Timeout:=0) {

        oldDHW:= DetectHiddenWindows(True)
        oldSMM:= SetTitleMatchMode(2) ; contains (default)

        TargetHWND := this.SenderHWND

        ;MsgBox this.SenderHWND, "Messenger Reply"

        if !TargetHWND
            throw Error("Sender window does not exist.", -1)
