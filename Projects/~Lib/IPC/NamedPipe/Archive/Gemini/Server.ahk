...
; COMMENT OUT WHEN RUN WITH TASK SCHEDULER
if !A_IsAdmin {
...
;myPipe := PipeHelper("\\.\pipe\AHK_LocalUTF16Pipe").RunServer(MyWork)
;myPipe := PipeHelper("\\.\pipe\GLOBAL\AHK_GlobalUTF16Pipe").RunServer(MyWork)

myPipe := PipeHelper().RunServer(MyWork)
