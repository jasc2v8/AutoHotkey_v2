...
class AdminLauncher {
    ...
    Listen(PipeName:=this.PipeName) {
        ...
        ; path = (workerPath, SyncBackPath, SyncBackProfile)
        this.logger.Write("GetExePath: " this.GetExePath(path))

        ;Run(path)
        RunShell(this.GetExePath(path))

        ;this.Send("ACK: " path, "REPLY")
        ;this.Send("ACK: " path)

        return true
    }

    ; if ahk then run A_AhkPath . path.ahk, else run path.exe
    GetExePath(path) {
        split := StrSplit(path, ",")
        if (SubStr(split[1], -4) = ".ahk")
            path := A_AhkPath . "," . path
        return path
    }
