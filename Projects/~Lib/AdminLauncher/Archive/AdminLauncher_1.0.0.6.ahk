...
/*
    TODO:
        Do NOT implement AutoStart, stick with StartTask (much simpler)
*/
...
If (A_LineFile == A_ScriptFullPath) {
    ...
    logger2 := LogFile("D:\cmdLine.log", "cmdLine", true)
    ...
    ;
    ; if no args from command line then listen for request
    ;

    if (A_Args.Length > 0 ) {

        ;launcher:= AdminLauncher("AdminLauncher", "AdminLauncherPipe2")
        ;launcher:= AdminLauncher()
        ...
        ;DetectHiddenWindows true
        ;WinWait("AdminLauncher.ahk ahk_class AutoHotkey")
        ...
        logger2.Write("cmdLine: " cmdLine)

        ;
        ; Run the cmdLine
        ;
        launcher.Send(cmdLine)

        logger2.Write("SENT: " cmdLine)
