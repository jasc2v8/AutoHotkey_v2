...
class ProcessManager {
    ...
    Status() {
        ...
        ; Check if the process is still actually there
        currentPID := ProcessExist(this.ProcessName)
        if (currentPID = 0)
        {
            return "Process Lost (Cleaning up...)"
        }
        
        return "Monitoring " . this.ProcessName . " (PID: " . currentPID . ") | Active for " . DateDiff(A_Now, this.StartTime, "Seconds") . "s"
    }
