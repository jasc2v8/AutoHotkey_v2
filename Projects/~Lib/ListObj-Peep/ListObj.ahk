; 2025-12-30 jasc2v8
;   Changed Peep to ListObj and Saved as ListObj
; 2025-12-26 jasc2v8
;   static gui_h := 640 ; was 1000
;   bh:=30 ; was 40
;   con.SetFont('s9 c' font_color , 'Segoe UI') ; was s10 Default
;   if Is_Admin then unpause works the same as continue, see unpause(*) ; can't unpause if Admin

/**
 * @description {@link https://github.com/GroggyOtter/ListObjAHK/|`ListObj()`}
 ...
 * - `true` = [Default] Code execution will pause when the ListObj() GUI is shown. This acts similarly to how MsgBox() works.  
 *   Code is resumed when Continue or Unpause is clicked.
 * - `false` = Code execution continues immediately after displaying the ListObj() GUI.
 ...
 * - `Positive number` = [Default] Use the built-in GUI to display ListObj() information.  
 * - `Negative number` = Use AHK's {@link https://www.autohotkey.com/docs/v2/lib/MsgBox.htm|MsgBox()} to display ListObj() information.  
 * - `0` = [Default] No information is displayed but a ListObj object is returned. The text can be retrieved by calling it.  
 * 
 *     ListObj_obj := ListObj(data)
 *     MsgBox(ListObj_obj())
 ...
 * Default: A_ScreenWidth / 2 - (ListObj.gui_w/2)
 * @property {Number} gui_y  
 * The default starting height for the GUI.  
 * Default: A_ScreenHeight / 2 - (ListObj.gui_h/2)
 * @returns {ListObj}
 * A ListObj object is always returned.  
 * The ListObj text can be retrieved by calling the returned object or accessing the `Text` property.  
 * 
 *     item := [1,2,3]
 *     pobj := ListObj(item)
 *     MsgBox(pobj())
 *     Msgbox(pobj.Text)
 * @example <caption>Multiple ListObj() examples.</caption>
 * #Include ListObj.v2.ahk
 * 
 ...
 * )
 * ListObjobj := ListObj(item)
 * ; To view ListObj text again, call the returned object.
 * MsgBox(ListObjobj())
 */
class ListObj {
     ...
     * If true, code execution will pause when the ListObj() GUI is shown. This acts similarly to how {@link https://www.autohotkey.com/docs/v2/lib/MsgBox.htm|MsgBox()} works.  
     * If false, code execution continues immediately after displaying the ListObj() GUI.
     * Default is: 1
     */
     ...
     * If set to a positive number, ListObj's custom GUI is used to display information.  
     * If set to a negative number, the standard MsgBox() is used.  
     * If set to zero, nothing is displayed. A ListObj object is still returned for text retrieval. 
     * Default is: 1
     */
     ...
     * Default is: 1000
     */
    static gui_h := 640 ; 1000
    /** @prop {Number} gui_x  
     * The default starting X coordinate.  
     * This is the x value of the upper left corner.  
     * Default is: A_ScreenWidth / 2 - (ListObj.gui_w/2)
     */
    static gui_x := A_ScreenWidth / 2 - (ListObj.gui_w/2)
    /** @prop {Number} gui_y  
     * This is the y value of the upper left corner.  
     * Default is: A_ScreenHeight / 2 - (ListObj.gui_h/2)
     */
    static gui_y := A_ScreenHeight / 2 - (ListObj.gui_h/2)
    ...
    static reset_to_default() {
        ...
        this.gui_x := A_ScreenWidth / 2 - (ListObj.gui_w/2)
        this.gui_y := A_ScreenHeight / 2 - (ListObj.gui_h/2)
        this.gui_h := 1000
        this.gui_w := 600
        this.gui_title := ''
    }
    ...
    __New(Title:='', items*) {
        ...
        if (items.length < 1) {
            MsgBox('No items were provided to ListObj()')
            return ''
        }

        this.gui_title:=Title
        ...
        for value in items {
            ...
            else txt .=  ListObj.unset_value1
            txt .= "`n`n"
        }
        txt := Trim(txt, '`n') '`n'
        ; Display text
        if (ListObj.display_with_gui > 0)
            this.gui_display(txt)
        else if (ListObj.display_with_gui < 0)
            MsgBox(txt)
    ...
    extract(item, ind) {
        ...
        switch {
            ...
            case (item is Primitive):
                if ListObj.include_prim_type
                    data .= '<' StrUpper(item_type) '> '
                if (item_type = 'String')
                    if (ListObj.include_string_quotes > 0)
                        data .= '"' item '"'
                    else if (ListObj.include_string_quotes < 0)
                        data .= "'" item "'"
                    else data .= item
                else data .= item
                return data
            ; Var Refs
            case (item is VarRef):
                return ListObj.var_ref_value
            ; COM values
            case ListObj.is_com_type(item_type):
            ...
            case ((item is Object) && this.is_duplicate(item)):
                return ListObj.duplicate_ref_value '<Type: ' item_type '>'
            ; The "I f*@ked up" clause
            case !IsObject(item):
                return MsgBox('GroggyOtter didn`'t account for this.'
                    '`nVisit the ListObj() GitHub page and let him know '
                    'know he missed something up. The item type is:`n' item_type)
        }
        
        ind2 := ind ListObj.indent_str
        ind3 := ind2 ListObj.indent_str
        
        ; Handle built-in props and own props
        if ListObj.include_built_in
        ...
        switch {
            ; Assemble array data
            case (item is Array):
                not_inline := (ListObj.include_built_in || !ListObj.array_values_inline || own_props != '' || !this.all_elements_are_prim(item))
                header := item_type '['
                for index, value in item {
                    if not_inline {
                        if ListObj.include_array_index
                            if ListObj.key_value_inline
                                data .= '`n' ind2 index ': '
                            else data .= '`n' ind2 index ': `n' ind3
                        else if ListObj.key_value_inline
                            data .= '`n' ind2
                        else data .= '`n' ind2
                    }
                    if IsSet(value)
                    ...
                    else data .= ListObj.unset_value
                    if ListObj.include_end_commas
                        data .= ', '
                }
                this.trim_end_comma(data)
                if not_inline
                    end_cap := '`n' ind
                        . (ListObj.key_value_inline ? '' : ind)
                        . (ListObj.indent_closing_tag ? ind2 : '')
                        . ']'
                else end_cap := ']'
            ...
            case (item is Gui.Control):
                ...
                data .= ListObj.include_built_in ? this.add_built_in(item, 'Gui.Control', ind2) : ''
            ...
            case ListObj.is_enumerable(item):
    ...
    add_built_in(item, typ, ind, is_gui_con:=0) {
        ...
        if is_gui_con
            typ := 'Gui.Control'
        for k, prop in ListObj.props[typ] {
    ...
    make_gui() {
        ...
        m           := 5
        bh          := 30
        bw          := (gw - m * (ListObj.btn_data.Length + 1)) / ListObj.btn_data.Length
        edw         := gw - m * 2
        edh         := gh - m * 3 - bh
        bg_color    := ListObj.dark_mode ? '101010' : 'F0F0F0'
        font_color  := ListObj.dark_mode ? 'D8D8D8' : '101010'
        happy       := 420
        
        ; Gui
        goo := Gui('+Resize -DPIScale +MinSize400x200', 'ListObj(' this.gui_title ')', this)
        goo.btn_h := bh
        ...
        for k, btn in ListObj.btn_data {
            ...
            con.SetFont('s9 c' font_color , 'Segoe UI')
    ...
    gui_resize(goo, *) {
        ...
        bh := goo.btn_h
        by := gui_h - pad - bh
        bw := (gui_w - pad * (ListObj.btn_data.Length + 1)) / ListObj.btn_data.Length
        for control in goo
            switch control.Type {
                case 'Button':
    ...
    gui_display(txt) {
        ...
        this.gui.show('x' ListObj.gui_x ' y' ListObj.gui_y ' w' ListObj.gui_w ' h' ListObj.gui_h)
        control := this.get_default_gui_btn()
        this.gui[control].Focus()
        if ListObj.gui_pauses_code
            Pause ; pause 1 If omitted, the current thread is paused. (helps if Admin)
    }
    
    get_default_gui_btn() {
        name := ListObj.default_gui_btn
    ...
    save_to_clip(*) {
        ...
        TrayTip('Text copied to clipboard', 'ListObj(AHK)', 0x10)
    }
