...
Class GuiLayout {
    ...
    static Fill(MyGui, Control, Right:=0, Bottom:=0 ) {
        ...
        titleBar := client.Y - gui.Y - gui.MY
        clientRight := gui.W - (gui.MX*2) - (ctrl.MX*2) + 4
        clientBottom :=	gui.H - TitleBar - (gui.MY*2) - (gui.MY*2) + 4

        if (Right = 0) {
            NewWidth:= clientRight
        } else { 
            NewWidth := gui.W-(gui.MarginX*3) - Right
        } 

        if (Bottom = 0) {
            NewHeight := clientBottom
        } else { 
            NewBottom := gui.H-(gui.MarginY*2) - Bottom
        } 
       
        ;Control.Move(,,gui.W-(gui.MarginX * 2))
        Control.Move(,,NewWidth, NewHeight) ;320
