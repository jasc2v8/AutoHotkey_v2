    ...
    GetClientPos(GuiObj:=this.GuiObj) {
       ...
       ; BottomBorder    : bottomBorder,
        ...
        return {
            ...
            Top     : border,
            Bottom  : clientRectBottom - guiBorderHeight,

            L       : windowBorderX + border,
            R       : clientRectWidth - border,
            T       : border,
            B       : clientRectBottom - guiBorderHeight,
...
__GuiLayout_Test() {
    ...
    Test2() {
        ...
        y:= L.Pos(C).Top
        h:= L.Pos(C).Bottom - L.Margin ; L.Pos(C).Height- L.Pos(C).Top
        x:= L.Pos(C).CenterX
        myX1.Move(x, y, , h)  ;Blue
        myY1.Move(L.Pos(C).Left, L.Pos(C).CenterY, L.Pos(C).Width - L.Margin, )  ;Yellow
    }
}
