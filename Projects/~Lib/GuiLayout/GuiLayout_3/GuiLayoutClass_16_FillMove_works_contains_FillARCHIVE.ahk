...
/*
    TODO:
        ...
        Note the TODO for Class variables below
            this.Margin

            this.GlobalRedraw
            Redraw := (GlobalRedraw) ? True : False
...
Class GuiLayout {
    ...
    ;TODO Add to Move, MoveFill, MoveRow
    ; Redraw := (GlobalRedraw) ? True : False
    GlobalRedraw := False
    ...
    Fill_ARCHIVE(MyControl, newX:="", newY:="", fillX:="", fillY:="", Redraw:=False ) {
    ...
    ;ListVars
        ...
        x := (newX = "") ? this.Pos(MyControl).X : newX -= this.Pos(MyControl).x + this.Pos(MyControl).Width
        MyControl.Move(calcX)
        ...
        ; calculate the delta W and extend

        w := (fillX != "") ? fillX : this.Pos(MyControl).Width
        MyControl.Move(, , w, )

        h := (fillY != "") ? fillY - this.Pos(MyControl).ControlY : this.Pos(MyControl).ControlHeight
        MyControl.Move(, , , h)
        ...
        ;calcX += 0 ; this.Margin + this.Margin ; this.Pos(MyControl).W 
    ...
    ; x,y with values relative to another control or client: Use FillMove or Move()
    ; x,y with absolute values: Use CtrlObj.Move()
    Move_OLD(MyControl, newX:="", newY:="", newW:="", newH:="", Redraw:=False ) {

        if (Type(MyControl) = "String")
            MyControl := this.GuiObj[MyControl]

        x := (newX = "") ? this.Pos(MyControl).X : this.Pos(MyControl).Width + this.Margin*2

        y := (newY != "") ? this.Pos(MyControl).Y : this.Pos(MyControl).ControlHeight

        w := (newW = "") ? this.Pos(MyControl).W : this.Pos(MyControl).W + this.Margin*2

        h := (newH != "") ? this.Pos(MyControl).H : this.Pos(MyControl).ControlHeight

        MyControl.Move(x, y, w, h)
        ...
        ; ;calcX -= this.Pos(MyControl).X - this.Margin*1 - 200
        ; calcX -= newX - this.Margin*1 - 200
        ; MyControl.Move(calcX, calcY)
        ...
        if (newX != "") {
            ...
            deltaX := calcX - this.Pos(MyControl).X - this.Margin*1
            deltaY := calcY - this.Pos(MyControl).Y - this.Margin*1
 ...
 ;not this is control, not client.clientRight := this.Pos(this.GuiObj).Right
        ...
        ;TODO: TEST ALL OF THESE
            ...
            Right   : OutX + OutWidth + this.Margin, ;OK 
            R       : OutX + OutWidth, ;OK 
            Top     : OutY,
            T       : OutY,
        }
...
__GuiLayout_Test() {
    ...
    ;return
            ...
            ; return
                ...
                ;x:= L.Pos("Divider").Right ; - ButtonWidth - L.Margin
                ;x:= L.Pos().Right
                ;y:= L.Pos("Edit").Top
                ;L.MoveFill("Open", x, y, , ,)
                ;L.Move("Open", x, y, , )
