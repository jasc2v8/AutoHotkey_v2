...
Class GuiLayout {
    ...
    static Row(MyGui, Controls, Layout, oldY:=-1) {
        ...
        for control in Controls
             GuiLayout.ControlMove(Control,,newY)
...
__GuiLayout_Test() {
    ...
    Test1() {
        ...
        OnButton_Click(*) {
            ...
            Controls := WinGetControls(MyGui)

            list := "Name, Type, ClassNN, Text`n-----------------------------`n`n"
            For ctrl in Controls {
                nop:=true
                list .= MyGui[ctrl].Name ", " MyGui[ctrl].Type ", " MyGui[ctrl].ClassNN  ", " MyGui[ctrl].Text "`n`n"
                ;list .= ControlGetText(ctrl)  "`n`n"
                ;list .= ctrl "`n`n"
            }

            bool := Type(Controls) = "Array" ? "TRUE" : "FALSE"

            MsgBox(list, "IsArray: " bool)
