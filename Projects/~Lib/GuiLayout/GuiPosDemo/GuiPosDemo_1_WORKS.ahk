...
/*
    ...
    X := L.Pos().Right  - bW         =>   L.Pos().Right  =>  L.Move(myGuiCtrl, L.().Right)

    Y := L.Pos().Border + L.Margin   =>   L.Pos().Left

    X := L.Pos().Width  - bW - bW - L.Margin    => L.Pos().Right  - bW - bW
...
Demo2() {
    ...
    AddControls() {
        ...
        ; x:=5+9+38+9 ; 61: 5 window border + 9 margin + 38 button width + 9 margin
        ; y:= 0
        x:= L.Pos().Left + L.Margin + bH + L.Margin
