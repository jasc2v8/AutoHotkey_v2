...
Class GuiLayout {
    ...
    static Row(Controls, Layout, newX:="", newY:="", newW:="", newH:="", Redraw:=False) {
        ...
        if (Layout = "AlignRight") {
            ...
            While (index > 0)
            {
                ...
                ;oldX := ctrl.X
                ;calcX := grui.W-grui.MarginX-(grui.MarginX/2)-A_Index*(ctrl.W+grui.MarginX)
                ; newX := (newW="") ? ctrl.X : grui.W-grui.MarginX-(grui.MarginX/2)-A_Index*(ctrl.W+grui.MarginX)

                ; newY := (newY="") ? ctrl.Y : newY
                ; newY := (newY=0 ) ? client.Bottom : newY

                ; newW := (newW="") ? ctrl.W : newW
                ; newW := (newW=0 ) ? 200 : newY              ; CALC WITH BASED ON TEXT LENGTH???

                ; newH := (newH="") ? ctrl.H : newH

                ; if (newY="")
                ;     newY := ctrl.Y
                ; else if (newY=0)
                ;     newY := client.Bottom - ctrl.H - grui.MarginY
    
                if (A_Index=1) {
                    xSpace := ctrl.W  + grui.MarginX
                    xPos := client.Right - ctrl.W + grui.MarginX - 0
                }
                else
                    xPos -= xSpace
                ...
                newX := (newX = "") ? ctrl.X : (newX = 0) ? xPos : xPos

                newY := (newY = "") ? ctrl.Y : (newY = 0) ? (client.Bottom - grui.MarginY) : newY

                newW := (newW = "") ? ctrl.W : newW ; (newX = 0) ? newX : newX ; CALC WIDTH BASED ON TEXT LENGTH???

                newH := (newH = "") ? ctrl.H : (newH = 0) ? (client.Bottom - ctrl.H - grui.MarginY) : newH

                ; new? ctrl.Y : (newY = 0) ? (client.Bottom - ctrl.H - grui.MarginY) : newY
...
;object or ClassNN?
            ...
            ; if ctrl.Value = "", use ctrlText? or vice versa
            ; for ctrl in Buttons
            ...
            ButtonRow := GuiLayout.Row(Buttons, "Right", x:=0, y:=0, w:="", h:="", Redraw:=False)
