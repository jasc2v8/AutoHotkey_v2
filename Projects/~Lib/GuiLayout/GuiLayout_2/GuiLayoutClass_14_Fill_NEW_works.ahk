...
Class GuiLayout {
    ...
    Fill_NEW(MyControl, newX:="", newY:="", newW:="", newH:="", Redraw:=False ) {
        ...
        y := (newY = "") ? this.Pos(MyControl).Y : newY ;-= this.Pos(MyControl).y + this.Pos(MyControl).Height
        MyControl.Move(, y)

        w := (newW != "") ? newW : this.Pos(MyControl).Width
        MyControl.Move(, , w, )
