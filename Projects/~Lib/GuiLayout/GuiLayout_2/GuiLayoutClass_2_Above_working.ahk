    ...
    MoveRow(Controls, Layout, newX:="", newY:="", newW:="", newH:="", Redraw:=False) {
        ...
        if (Layout = "AlignRight") {
            ...
            While (index > 0)
            {
                ...
                if (A_Index=1) {
                    xSpace := this.Pos().ClientRight - calcW - this.Pos().BorderWidth
...
__GuiLayout_Test() {
    ...
    ;MyGui.OnEvent("Size", Gui_Size)
        ...
        OnButton_Click(*) {
            ...
            ; Move Divider to above the Button row
            L.Move("Divider",, L.Pos("Cancel").Above, L.Pos().ClientRight)

            ; Fill Text to the top of the Divider
            newH := L.Pos("Divider").Above - L.Pos("Text").ControlY
            L.Move("Text",,, w:=L.Pos(MyGui).ClientRight, newH)

            ; Fill Edit to the width of the client area
            L.Move("Edit", x:="", y:="", w:=L.Pos(MyGui).ClientRight)
