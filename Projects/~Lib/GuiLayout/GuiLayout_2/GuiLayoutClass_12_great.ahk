...
Class GuiLayout {
    ...
    Fill(MyControl, FromX, ToX, FromY, ToY) {
        ;TODO

        ; if (this.AutoWidth) {

        ;     ;w:= L.Pos("Text").X ; assumes this.AutoSize=True
        ;      ;''   L.Move("Divider", , y, w,)

        ;     calcW := this.Pos().Right - calcX +8
        ;     ;calcW := this.Pos().Right - this.Pos(MyControl).x
    ...
    Move(MyControl, newX:="", newY:="", newW:="", newH:="", Redraw:=False ) {
        ...
        ; MyControl.Move(calcX, calcY, calcW)
        ; if (Redraw)
        ;     WinRedraw(MyControl.Gui)
        ; return

        ; This order allows all x,y,w,h params in one Move call
        ; This help preserve the pos values for revercing other control pos

        _W(MyControl, calcX, calcY, calcW, calcH)

        _XY(MyControl, calcX, calcY, calcW, calcH)
    ...
    Move_WORKS(MyControl, newX:="", newY:="", newW:="", newH:="", Redraw:=False ) {
...
__GuiLayout_Test() {
    ...
    ;return
            ...
            try {
                ...
                switch TestNumber {
                    case 1:
                        L.MoveRow(Buttons, "LEFT", x, y, w, h, ForceReDraw)
                    case 2:
                        L.MoveRow(Buttons, "CENTER", x, y, w, h, ForceReDraw)
                    case 3:
                        L.MoveRow(Buttons, "RIGHT", x, y, w, h, ForceReDraw)
                    case 4:
                        L.MoveRow(Buttons, "FILL", x, y, w, h, ForceReDraw)
                }

                ; Move the divider to above the buttons and extend to the client right
                y:= L.Pos("Cancel").Above - L.Pos("Divider").H
                w:= L.Pos().Right - L.Pos().Left + 4 ; L.Pos("Text").X
                L.Move("Divider", , y, w,)

                ;L.Fill("Divider", , L.Pos().Right) 
                ...
                x:= L.Pos("Divider").Right - L.Pos("Cancel").Width  ;- L.Pos().Margin*HackValue
                y:= L.Pos("Edit").Top
                h:= L.Pos("Edit").Height+1
                L.Move("Open", x, y, ButtonWidth, h)
