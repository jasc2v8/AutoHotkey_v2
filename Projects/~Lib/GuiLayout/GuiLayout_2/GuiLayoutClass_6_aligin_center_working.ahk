    ...
    MoveRow(Controls, Layout, newX:="", newY:="", newW:="", newH:="", Redraw:=False) {
        ...
        if (Layout = "AlignCenter") {
            ...
            for MyControl in Controls
            {
                ...
                ; --- GUI Creation ---
                ; MyGui := Gui(, "Centered Buttons (Fixed Gap)")
                ; MyGui.SetFont("s10")

                ; Loop to create and position the 4 buttons
                Loop NumBtns
                {
                    ...
                    ; Add the button using the calculated XPos and fixed width
                    ;MyGui.Add("Button", "x" XPos " w" BtnWidth " h30", "Btn " i)
                    
                    aControl := Controls[A_Index]
                    ; Add the button using the calculated XPos and fixed width
                    ;MyGui.Add("Button", "x" XPos " w" BtnWidth " h30", "Btn " i)
                    aControl.Move(xPos, ctrlY, ctrlW, ctrlH)
                }

                ; ; Set the Gui width to match the RowWidth (580) for visual confirmation
                ; ;MyGui.Show("w" RowWidth)
                ...
                ; GapMargin := TotalSpace / (NumBtns + 1) ; 320 / 5 = 64

                ; ; The width of a button plus one gap
                ; Step := BtnWidth + GapMargin ; 65 + 64 = 129

                ; ; --- GUI Creation ---
                ; ;MyGui := Gui(, "Centered Buttons")
                ; ;MyGui.SetFont("s10")
                ...
                ; ; Loop btnCount {
                ; ;     x := xStart + (A_Index - 1) * (btnWidth + btnSpacing)
                ; ;     ;Gui.Add("Button", "x" x " y10 w" btnWidth, "Button " A_Index)
                ; ;     MyControl.Move(x, ctrlY, ctrlW, ctrlH)
                ; ; }

            }
        }
        if (Layout = "AlignCenter-fill-sorta") {
        ...
        if (Layout = "AlignCenter-COPILOT") {
            ...
            for MyControl in Controls
            {
                ...
                groupWidth := btnCount * btnWidth + (btnCount - 1) * btnSpacing

                xStart := (totalWidth - groupWidth) // 2
        ...
        if (Layout = "AlignCenter-OLD") {
            ...
            for MyControl in Controls
            {
                ...
                if (A_Index=1) {
                    xPos := (this.Pos().W / 2) - (ctrlW - 8)
                } else
                    xPos += this.Pos().Margin + ctrlW
        ...
        if (Layout = "AlignFill") {
            ...
            for MyControl in Controls {
                ...
                ; buttonSpaceTotal := Round(this.Pos().W / Controls.Length)
                ; buttonSpace := buttonSpaceTotal / Controls.Length
                ; buttonMargin := (buttonSpace - CtrlW) / 2
                ; buttonSpaceCenter := buttonSpace / 2
                ; xPos := buttonSpaceCenter - ((buttonMargin+ctrlW+buttonMargin) * Controls.Length)

                gap := 64
                btnWidth := 65

                Loop Controls.Length {
                    xPos := gap * A_Index + btnWidth * (A_Index - 1)
                    MyControl.Move(xPos, ctrlY, ctrlW, ctrlH)
                }

                ; if (A_Index=1)
                ;     xPos := (buttonSpaceCenter - Controls.Length) / 2
                ; else
                ;     xPos += buttonMargin + CtrlW
