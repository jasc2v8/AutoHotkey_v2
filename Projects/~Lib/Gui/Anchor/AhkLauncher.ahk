...
OnGui_Size_TEST(GuiObj, MinMax, Width, Height) {
    if (MinMax = -1)
        return

    ; 1. Resize the Search Box (Width only)
    Anchor(MyEdit, "w")

    ; 2. Resize the Tab Control (Width and Height)
    Anchor(Tabs, "wh")

    ; 3. Resize all ListViews together (Width and Height)
    ; DeferWindowPos handles the flicker so we don't need .Visible := false
    Anchor([LV1, LV2, LV3, LV4], "wh")

    ; 4. Handle bottom buttons
    ; Move Refresh and Explore with the bottom (Y)
    Anchor([Gui1["Refresh"], Gui1["Explore"]], "y")
    
    ; Move Checkbox and Hide button (X and Y) 
    ; Using "xy" keeps them relative to the bottom-right corner
    Anchor([MyCheckBox, Gui1["Hide"]], "xy")
}
...
OnGui_Size(GuiObj, MinMax, Width, Height) {
    ...
    Anchor(MyEdit, "w", redraw)

    Anchor(Tabs, "wh", redraw)

    Anchor(LVArray, "wh", redraw)

    static bottomRowArray := [Gui1["Refresh"], Gui1["Explore"], MyCheckBox, Gui1["Hide"]]
    Anchor(bottomRowArray, "y", redraw)
    ...
    SetTimer () => SetListViewsVisible(true), -100

}

SetListViewsVisible(TrueFalse:=true) {
    LV1.Visible := TrueFalse
    LV2.Visible := TrueFalse
    LV3.Visible := TrueFalse
    LV4.Visible := TrueFalse
}
