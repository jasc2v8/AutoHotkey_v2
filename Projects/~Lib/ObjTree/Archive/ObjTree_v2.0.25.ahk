...
; ObjTree v2.0.25
; Re-added Save to File with stable Scroll-to-top logic
...
ObjTree(Obj, Title := "Object TreeView", IsModal := true) {
    ...
    filterEdit := myGui.Add("Edit", "x+5 vFilter w350")
    ...
    ; Event for filtering as you type
    ...
    myGui.OnEvent("Size", (guiObj, minMax, width, height) => ObjTree_Size(guiObj, minMax, width, height, TV, btnOk, filterEdit))
...
ObjTree_Size(GuiObj, MinMax, Width, Height, TV, btnOk, filterEdit) {
    if (MinMax = -1)
        return
    
    filterEdit.Move(,, Width - 65)
