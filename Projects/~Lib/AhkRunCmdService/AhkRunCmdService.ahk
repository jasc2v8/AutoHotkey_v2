...
; RUNNING STANDALONE, NOT AS A SERVICE, REQUIRES RUNAS ADMIN FOR SYNCBACK
...
; the ServiceLog.txt file is created with Admin
; perhaps create a common log file and grant users full control?
global ServiceLogFile:= "D:\ServiceLog.txt"

global SF := SharedFile("Server")

Run 'icacls ' SF.SharedFilePath ' /grant "Everyone:F'
; inop as a Service? Run 'icacls ' SF.SharedFilePath ' /grant ' A_UserName ':F'
...
CheckMessages() {
    ...
    ; commands supported are Array, CSV, EXE, or CMD
    command:= SF.Read()
