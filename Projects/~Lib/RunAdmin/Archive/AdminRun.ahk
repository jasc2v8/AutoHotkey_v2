; TITLE  :  AdminRun v1.0.0.8
; SOURCE :  AHK Forums and jasc2v8
; LICENSE:  The Unlicense, see https://unlicense.org
; PURPOSE:  Run any program elevated without the UAC prompt.
; SUMMARY:  Controller.ahk starts TaskName 'AdminRun', who runs AdminRun.ahk, who then runs Worker.ahk.
; USAGE  :  On-Demand Task must be in Task Scheduler: Name=AdminRun, Target=AdminRun.ahk.
;           Two Scripts: Controller and Worker.
;           Controller Launches Worker Elevated, Worker perofrms work and replies when finished.
; FLOW   :  Controller.exe StartTask AdminRun, launches AdminRun.ahk at runLeve='highest'
;           AdminRun.Listen()
;        	Controller.exe Send(Worker.exe, Params) to AdminRun
; 	        AdminRun.RunWait(Worker.exe, Params)
; 	        AdminRun.Send(reply)
...
class AdminRun {
    ...
    logger := LogFile("D:\AdminRun.log", "AdminRun", true)
    TaskName := "AdminRun"
    PipeName := "AdminRunPipe"
    ...
    ;     if WinExist("AdminRun.ahk ahk_class AutoHotkey") {
    ;         TargetPID := WinGetPID()
