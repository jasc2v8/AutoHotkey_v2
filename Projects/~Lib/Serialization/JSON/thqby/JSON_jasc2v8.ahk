; 2025-12-26 jasc2v8 added ToObject() and ToString()

/************************************************************************
 * @description: JSON格式字符串序列化和反序列化, 修改自[HotKeyIt/Yaml](https://github.com/HotKeyIt/Yaml)
 * 增加了对true/false/null类型的支持, 保留了数值的类型
 * @translation: JSON string serialization and deserialization. Modified from HotKeyIt/Yaml.
 * Added support for true / false / null types and preserved numeric data types.
 * @author thqby, HotKeyIt
 * @date 2025/12/22
 * @version 1.0.8
 * https://github.com/thqby/ahk2_lib/blob/master/ws-min.ahk
 ***********************************************************************/
class JSON {
	static null := ComValue(1, 0), true := ComValue(0xB, 1), false := ComValue(0xB, 0)

  	static ToObject(text, keepbooltype := false, as_map := true) {
        return this.parse(text, keepbooltype := false, as_map := true)
    }
    ...
    static ToString(obj, expandlevel := unset, space := "  ") {
        return this.stringify(obj, expandlevel := unset, space := "  ")
    }
