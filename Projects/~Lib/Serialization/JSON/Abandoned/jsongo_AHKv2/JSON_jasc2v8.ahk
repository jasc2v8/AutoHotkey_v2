/**
 * 2025-12-30, jasc2v8
 *  rename class jsongo to JSON
 *  added: ToString(), ToObject()
 *  two lines changed to: remove := "JSON.JSON_Remove()" ;JSON.JSON_Remove() (only affects the error_log)
 * 
...
class JSON {
    ...
    static ToObject(jtxt, reviver:='') => this._Parse(jtxt, reviver)
     ...
     * json := JSON.Stringify(obj, , 4)
     * MsgBox(json)
     */
    static Stringify(base_item, replacer:='', spacer:='', extract_all:=0) => this._Stringify(base_item, replacer, spacer, extract_all)
    static ToString(base_item, replacer:='', spacer:='', extract_all:=0) => this._Stringify(base_item, replacer, spacer, extract_all)
    ...
    static _Parse(jtxt, reviver:='') {
        ...
        , key    := ''
        , is_key := 0
        , remove := "JSON.JSON_Remove()" ;JSON.JSON_Remove()
        this.error_log := ''
