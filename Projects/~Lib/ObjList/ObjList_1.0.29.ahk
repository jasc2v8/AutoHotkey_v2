...
class ObjList {
    ...
    __New(InputData, Title := "List Manager", IsModal := true) {
        ...
        ; Mode Toggle Button - Width reduced to 50
        this.BtnTheme := this.MyGui.Add("Button", "w50", this.IsDarkMode ? "Light" : "Dark")
    ...
    ApplyTheme() {
        if (this.IsDarkMode) {
            ...
            ; Advanced Dark Mode UI Calls
            this.SetDarkModeFrame(this.MyGui.Hwnd, true)
            DllCall("uxtheme\SetWindowTheme", "ptr", this.LV.Hwnd, "str", "DarkMode_Explorer", "ptr", 0)
        } else {
            ...
            ; Reset Advanced UI Calls
            this.SetDarkModeFrame(this.MyGui.Hwnd, false)
    ...
    OnSize(Width, Height) {
        ...
        ; Position Buttons - Shifted theme button for w50
        this.BtnAction.Move(Width - 80 - this.Margin, Height - 35)
        this.BtnTheme.Move(Width - 80 - this.Margin - 60, Height - 35)
