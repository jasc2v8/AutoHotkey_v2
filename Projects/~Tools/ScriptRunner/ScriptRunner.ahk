; TITLE  :  ScriptRunner v1.1.0.9
; SOURCE :  Gemini and jasc2v8
; LICENSE:  The Unlicense, see https://unlicense.org
; PURPOSE:  Edit and run CMD and Powershell scripts in a GUI
; NOTES  :  Version 1.1.1 updated 2026-01-06 - Added Clear History to Recent Files

;@Ahk2Exe-Set FileVersion, 1.1.0.9
;@Ahk2Exe-Set ProductVersion, 1.1.0.9
...
;global IniPath := A_ScriptDir "\ScriptRunner.ini"
...
; --- GUI Creation ---
MyGui := Gui("+Resize +MinSize520x400 +0x02000000", "Script Runner v1.1.0.9")
...
FileMenu.Add("Recent Files", RecentMenu)
...
UpdateRecentMenu() {
    ...
    hasFiles := false
    Loop 10 {
        ;path := IniRead(IniPath, "Recent", "File" A_Index, "")
        path := reg.Read("File" A_Index)
        if (path != "") {
            RecentMenu.Add(A_Index ". " path, (ItemName, *) => LoadFile(SubStr(ItemName, 4)))
            hasFiles := true
        }
    }
    ...
    if (hasFiles) {
        RecentMenu.Add()
        RecentMenu.Add("Clear History", (*) => ClearRecentHistory())
    } else {
        RecentMenu.Add("(Empty)", (*) => 0)
    }
}
...
ClearRecentHistory() {
    ...
    Loop 10
        reg.Delete("File" A_Index)
    UpdateRecentMenu()
    UpdateStatusBar("History cleared")
}

AddToRecent(path) {
    history := []
    Loop 10 {
        ;entry := IniRead(IniPath, "Recent", "File" A_Index, "")
        entry := reg.Read("File" A_Index)
        if (entry != "" && entry != path)
            history.Push(entry)
    }
    history.InsertAt(1, path)
    
    Loop Min(10, history.Length)
        ;IniWrite(history[A_Index], IniPath, "Recent", "File" A_Index)
        reg.Write("File" A_Index, history[A_Index])
...
LoadFile(path) {
    global FullPath
    if !FileExist(path) {
        UpdateStatusBar("File no longer exists")
        return
    }
    
    FullPath := path
    EditPath.Value := path
    EditPreview.Value := FileRead(path)
    UpdateTheme(path)
    ScrollPathToEnd()
    AddToRecent(path)
    EditPreview.Focus()
