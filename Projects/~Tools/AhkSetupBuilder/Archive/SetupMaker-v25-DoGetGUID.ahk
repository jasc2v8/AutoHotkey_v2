 ...
 */
;@Inno-AppId {{B7D1FB29-B701-4010-8DEA-A5477C60C76D}}
...
DoReadGuid(textFilePath) {
    ...
    try {
        ...
        while !file_obj.AtEOF {
            ...
            if InStr(line_content, findKey, true) {
                found_lines .= "Line " . line_number . ": " . line_content . "`n"
            }
        }
        file_obj.Close()
    } catch {
    ...
    startPos := InStr(found_lines, "{{")
    length := StrLen(guidFormat)

    GUID := SubStr(found_lines, startPos, length)

    MsgBox("line_number: " line_number "`n`n" .
    "found_lines: " found_lines "`n`n" .
    "GUID: [" GUID "]", "DEBUG")
