/*
    TODO:
    add checkbox "Increment Ahk Build Version" ?
*/
/**
 * AhkSetupBuiler -- Autohotkey and Inno Setup script compiler.
 * 
 * Features
 ...
 *  - A_MyDocuments "\AutoHotkey\AhkBuilder\Templates\"
 *  - A_MyDocuments "\AutoHotkey\AhkBuilder\Configs"
 *  - A_Temp "\AhkApps\AhkBuilder\AhkBuilder.ini"
 */
...
;@Ahk2Exe-Set ProductVersion, 0.0.0.0
 ...
 * the Ahk2exe compiler. This application reads this value and
...
global configPath := A_MyDocuments "\AutoHotkey\AhkBuilder\Configs\"
...
; #region GUI Create
MyGuiTitle := "AutoHotkey Setup Builder " MyProductVersionIntheGuiTitle
...
;DEBUG
;oldVer := "1.0.1.27"
;newVer := DoIncrementVersion(oldVer, "Build")
MsgBox("newVer: " newVer)
;DEBUG
...
DoCompileAhk() {
    ...
    version := DoReadSetting(editBoxAhk.Text,
            ";@Ahk2Exe-Set ProductVersion,")
    editBoxIssExe.Text := StrReplace(editBoxAhk.Text,
            ".ahk", "_setup_v" version ".exe")

    try {
        exitCode := RunWait(command, , "Hide")
        if (exitCode == 0) {
            WriteStatus("AhkExe created successfully: " ahkExePath iconMsg)
            DoIncrementProductVersion()
        } else {
...
DoCompileIss() {
    ...
    if !FileExist(innoExe) {
        MsgBox("Error: Inno Setup Compiler not found:`n`n" innoExe, "Error", "OK Icon!")
        return
    }
...
DoIncrementProductVersion() {
    ; get version from .ahk script MyScript_v0.0.0.0.ahk
    ; if no version, set version to 0.0.0.0
    ; else increment build number to 0.0.0.1
    ; read .ahk script
    ; replace ProductVersion, 
    ; write .ahk script to MyScript_v0.0.0.1.ahk
 
    ahkFile := editBoxAhk.Text

    if !InStr(SplitPath(ahkFile).NameNoExt, "_v") {
        ahkFile .= ".ahk"
    } else {
        oldVersion := DoReadSetting(ahkFile, "ProductVersion,")
        versionParts := StrSplit(versionLine, ".")
        build := versionParts[4]
        ;build += 1
        ...
        ahkText := FileRead(ahkFile)

        StrReplace(ahkText, versionLine, newVersionLine)
    ...
    versionFile := StrReplace(editBoxAhk.Text, ".ahk", ".version")

    if !FileExist(versionFile) {
        FileAppend("ProductVersion, 0.0.0.0", versionFile, "UTF-8")
    }

    versionLine := DoReadSetting(versionFile, "ProductVersion,")
    ...
    ;MsgBox("newVersion: " newVersion)

    FileDelete(versionFile)
    FileAppend("ProductVersion, " newVersion, versionFile, "UTF-8")
}
