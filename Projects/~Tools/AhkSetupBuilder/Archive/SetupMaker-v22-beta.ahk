/*
    TODO: Why inno compile fails the first time?
    Because all files are packed in the _setup.exe
    and the .iss file is in use?
*/
...
; #region Globals
;global configPath := A_MyDocuments "\AutoHotkey\AhkBuilder\Config\Config.ini"
global configPath := A_MyDocuments "\AutoHotkey\AhkBuilder\Config\Config.ini"
...
buttonSelectExe := MyGui.Add("Button", "xm w104 h24", "AHK &Exe")
    .OnEvent("Click", (*) => DoSelectFolder("EXE"))
...
DoCompileAhk() {
    ...
    if !DirExist(SplitPathObj(exePath).Dir) {
...
;DEL    outDir := SplitPathObj(exePath).Dir
...
DoSaveConfig() {
    ...
    WriteStatus("DEBUG Config path: " configPath)

    ; make sure path exists
    if !FileExist(configPath) {
        try {
            SplitPath(configPath, &FileName, &Dir)
            DirCreate(Dir)
        } catch Error as e {
            Throw("ERROR creating config Directory: " e.Message)
        }
    }

    ;DEL SelectedFileName := FileSelect("S16",, "Save File", "Config files (*.ini)")
    
    MyGui.Opt("+OwnDialogs")
    
    selectedFile := FileSelect("S16", configPath, "Save Config As...", "Config Files (*.ini)")

    if selectedFile {
        DoWriteIni()

        if (SplitPathObj(selectedFile).Ext != ".ini") {
            selectedFile .= ".ini"
        }
        
        try {
            FileCopy(iniPath, selectedFile, true) ; overwrite
            WriteStatus("Config Saved: " selectedFile)
        } catch Error as e {
            MsgBox("ERROR Saving Config File: " e.Message)
        }
    } else {
        WriteStatus("Nothing to save.")
    }
}
...
DoOpenConfig() {
    ...
    selectedFile := FileSelect("3", configPath, "Open Config File...", "Config Files (*.ini)")

    if selectedFile {
        try {
            FileCopy(selectedFile, iniPath, true) ; overwrite
            WriteStatus("Config Opened: " selectedFile)
            DoReadIni()
        } catch Error as e {
            Throw("ERROR Opening Config File: " e.Message)
        }
    } else {
        WriteStatus("Nothing to save.")
    }
