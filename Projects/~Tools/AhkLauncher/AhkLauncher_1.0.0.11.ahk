...
Gui1 := Gui("+Resize")
Gui1.Title:="AhkLauncher v1.0.0.11"
...
MyCheckBox      := Gui1.AddCheckBox("yp x+380 h24 Checked -Wrap vCheckBox", "Hide Gui")
...
ExternalScriptPath:= EnvGet('USERPROFILE') "\Documents\AutoHotkey\AhkLauncher\AhkApps\VSCodeSaveVersion.ahk"

^!v::
{   
    DetectHiddenWindows true
    if WinExist("VSCodeSaveVersion")
        WinActivate
    else if FileExist(ExternalScriptPath) {
...
OnGui_Size(GuiObj, MinMax, Width, Height) {
    ...
    Gui1.Opt("+MinSize480x320 +MaxSize1280x980")

    SetListViewsVisible(false)

    Anchor(MyEdit, "w", 
        [Tabs, LV1, LV2, LV3, LV4], "wh", 
        [Gui1["Refresh"], Gui1["Explore"]], "yr", 
        [MyCheckBox, Gui1["Hide"]], "xyr")
...
;----------------------------------------------------------------------------------
FileGetScriptVersion(ScriptPath) {
...
ListViewLoad(activeLV, LVMap) {
    ...
    for k, v in LVMap {
        ...
        if (SubStr(k,-3,3) = "ahk") {
            ...
            v := FileGetScriptVersion(p)
        }

        if (SubStr(k,-3,3) = "exe") {
            p := AhkAppsPath "\" k
            v := "v" FileGetProperty(p, "FileVersion")
        }
