;ABOUT: Version 1.0.3.0: Add HOtkey VScode, if BackupcontrolTool exits, activate, else run Syncback
...
TiteModeStartsWith  :=1
TiteModeContains    :=2 ; default
TiteModeExact       :=3
...
^!B::
{
    ...
    SetTitleMatchMode TiteModeStartsWith

    if WinExist("Backup Control Tool") {
        WinActivate("A") ; won't work on a script running as Admin unless this script is Admin.
        SoundBeep

    } else {
        Run(AhkAppsDir '\BackupControlTool.ahk')
    }
}
