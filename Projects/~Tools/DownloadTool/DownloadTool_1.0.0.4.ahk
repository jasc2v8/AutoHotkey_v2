...
;@Ahk2Exe-Set FileVersion, 1.0.0.4
...
;@Ahk2Exe-Set ProductVersion, 1.0.0.4
...
cmd:= runner.ToCSV("C:\Program Files\Private Internet Access\piactl.exe", "get", "connectionstate")

output := runner.RunWait(cmd)
...
StartVPN(){
  ...
  if !ProcessExist("pia-client.exe") {
    ...
    runner.Run("C:\Program Files\Private Internet Access\pia-client.exe")
...
ButtonStart_Click(Ctrl, Info) {
  ...
  if !StartURL() {
    WriteStatus("VPN and TOR started, but URL didn't open.")
  } else {
...
Sleep(1000)
 ...
 if output != "Disconnected" {
  runner.Run(PiaCtlPath ",disconnect")
 }
