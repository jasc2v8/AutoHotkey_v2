...
ExecuteScript(AsAdmin) {
    ...
    SplitPath(p, , &workingDir, &ext)
    
    isBatch := (StrLower(ext) = "bat" || StrLower(ext) = "cmd")
    enc := isBatch ? "UTF-8-RAW" : "UTF-8"
    
    ; Create temp execution file in the SAME directory to maintain relative path logic
    RunTemp := workingDir "\~usr_exec_temp." ext
    TempOut := A_Temp "\usr_out_temp.txt"
    
    try {
        FileOpen(RunTemp, "w", enc).Write(EditPreview.Value)
        
        EditPreview.Value := AsAdmin ? "--- RUNNING AS ADMIN ---" : "--- RUNNING ---"
        Prefix := AsAdmin ? "*RunAs " : ""
        
        ; Using A_ComSpec with the specific "" "" wrapping required for nested quotes in CMD
        if (isBatch) {
            RunWait(Prefix A_ComSpec ' /c ""' RunTemp '" > "' TempOut '" 2>&1"', workingDir, "Hide")
        } else {
            RunWait(Prefix 'powershell.exe -NoProfile -ExecutionPolicy Bypass -File "' RunTemp '" > "' TempOut '" 2>&1', workingDir, "Hide")
        }
        
        Sleep(400) ; Wait for file flush
        
        if FileExist(TempOut) {
            outData := FileRead(TempOut)
            EditPreview.Value := "--- OUTPUT ---`r`n`r`n" outData
            UpdateStatusBar("Ready")
            try FileDelete(TempOut)
        } else {
            EditPreview.Value := "--- NO OUTPUT RECORDED ---"
            UpdateStatusBar("Ready")
        }
        
        try FileDelete(RunTemp)
        SendMessage(0x0115, 7, 0, EditPreview.Hwnd)
        
    } catch Error as e {
        EditPreview.Value := "--- EXECUTION ERROR ---`n" e.Message
        UpdateStatusBar("Error")
        if FileExist(RunTemp)
            try FileDelete(RunTemp)
    }
}