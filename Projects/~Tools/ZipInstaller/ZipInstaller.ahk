...
; --- UI Setup ---
InstallGui := Gui("+Resize +MinSize700x600", "AHK Zip Installer (Admin)")
...
LogWin := InstallGui.Add("Edit", "xm w700 r18 ReadOnly +VScroll", "Log initialized...`r`n")
...
VerText    := InstallGui.Add("Text", "x620 yp+5 w80 Right", "v1.2.2") 

; --- Events ---
InstallGui.OnEvent("Size", Gui_Size)
...
; --- Dynamic Resizing ---
Gui_Size(thisGui, Minimized, Width, Height) {
    if (Minimized = -1) 
        return
    
    ZipPath.Move(,, Width - 170)
    BtnBrowse.Move(Width - 150)
    DestPath.Move(,, Width - 170)
    BtnFolder.Move(Width - 150)
    ProgBar.Move(,, Width - 20)
    
    LogHeight := Height - 420
    if (LogHeight < 100) 
        LogHeight := 100 
    LogWin.Move(,, Width - 20, LogHeight)
    
    BtnY := Height - 110
    InstallBtn.Move(, BtnY)
    ExportBtn.Move(, BtnY)
    ClearBtn.Move(, BtnY)
    CancelBtn.Move(, BtnY)
    
    OpenFolderBtn.Move(, BtnY - 65)
    StatusText.Move(, Height - 40, Width - 120)
    VerText.Move(Width - 90, Height - 35)
    
    DllCall("RedrawWindow", "Ptr", thisGui.Hwnd, "Ptr", 0, "Ptr", 0, "UInt", 0x0101)
}
...
StartInstallation(*) {
    ...
    try {
        ...
        AddLog("Total items to extract: " totalItems)

        counter := 0
        for item in items {
            counter++
            AddLog("(" counter " / " totalItems ") Extracting: " item.Name)
            destFolder.CopyHere(item, 4 | 16)
            
            ; Optional: Update Progress bar based on file count
            ProgBar.Value := (counter / totalItems) * 100
        }
