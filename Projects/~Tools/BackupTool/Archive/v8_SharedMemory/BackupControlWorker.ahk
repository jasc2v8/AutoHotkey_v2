...
SyncBackRequest  := RunAny.ConvertToCSV(SyncBackPath, SyncBackProfile)
SyncBackCommand  :=
...
logger.Write("SyncBackRequest: " SyncBackRequest)
    ...
    ;output:= RunAny(SyncBackRequest)

    appPath := "C:\Program Files (x86)\SyncBackSE\SyncBackSE.exe"
    param := "TEST"
    try {
        Run '"' appPath '" ' param
    } catch any as e {
...
;PostActionHandler(request)
    ...
    logger.Write("ACK: from Worker: " output)

    mem.Write("ACK: " output)
