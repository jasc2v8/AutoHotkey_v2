...
; SCRIPTS: BackupTool.ahk => RunTask.ahk => RunSkipUAC.ahk => BackupControlWorker.ahk

/*
  PSUEDOCODE:
  -----------
  BackupTool status "Running..."
  BackupTool task:=RunTask('AHK_RunSkipUAC')
  RunTask Exit

  On-Demand Task AHK_RunSkipUAC runs RunSkipUAC.ahk at runLevel='highest'
  On-Demand Task AHK_RunSkipUAC Exit
    RunSkipUAC listen for WorkerPath from BackupTool
    RunSkipUAC run WorkerPath at runLevel='highest'
    RunSkipUAC Exit

  BackupTool wait for RunSkipUAC to start
  BackupTool check if RunSkipUAC has created the SharedMemory object
  BackupTool WorkerPath to RunSkipUAC
  BackupTool wait for RunSkipUAC to write Reply

  BackupTool check if Worker has created the SharedMemory object

  BackupTool send BackupRequest to Worker
    ...
    Worker Exit
  BackupTool listen for Worker ACK
  BackupTool status "Finished.
*/
/*
  TODO:
...
;global LogPath         := EnvGet("PROGRAMDATA") "\AutoHotkey\BackupTool\BackupTool.log"
global LogPath          := "D:\BackupTool.log"
global logger           := LogFile(LogPath, "CONTROL")
logger.Disable()

global WorkerPath       := EnvGet("PROGRAMDATA") "\AutoHotkey\BackupTool\BackupWorker.ahk"
...
ButtonCommon_Click(Ctrl, Info){
  ...
  if (timedOut) {
    ...
    try
    {
      ...
      logger.Write("Wait for BackuplWorker to create pipe...")
