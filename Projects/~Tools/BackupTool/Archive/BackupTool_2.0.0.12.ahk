...
;@Ahk2Exe-Set FileVersion, 2.0.0.12
...
;@Ahk2Exe-Set ProductVersion, 2.0.0.12
...
ButtonCommon_Click(Ctrl, Info){
  ...
  if (timedOut) {
    ...
    launcher.Send(SyncBackPostAction)
    ;Sleep 2000
...
RunAsWorker() {
  ...
  global loggerWorker := LogFile(LogPath, "WORKER", true) ; true=Enable, false=Disable

  loggerWorker.Write("START BackupWorker")

  launcher:= AdminLauncher()
  SyncBackPostAction:= launcher.Receive()
  loggerWorker.Write("Received: " SyncBackPostAction)
