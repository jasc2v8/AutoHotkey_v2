...
;@Ahk2Exe-Set FileVersion, 2.0.0.14
...
;@Ahk2Exe-Set ProductVersion, 2.0.0.14
...
;global WorkerPath       := '"' A_AhkPath '"' A_Space "C:\Users\Jim\Documents\AutoHotkey\AhkLauncher\AhkApps\BackupTool.ahk"
...
logger.Write("START: Args: " A_Args.Length)
...
ButtonCommon_Click(Ctrl, Info){
  ...
  if (timedOut) {
    ...
    ;
    ; Start the Task AdminLauncher which runs AdminLauncher.ahk, which launches this script as /worker
    ;
    ...
    launcher.Send(WorkerPath) ; path containze /worker so skip if Exist
...
RunAsWorker() {
  ...
  global loggerWorker := LogFile(LogPath, "WORKER", false) ; true=Enable, false=Disable

  loggerWorker.Write("START BackupWorker")

  launcher:= AdminLauncher()

  BackupRequest:= launcher.Receive()

  loggerWorker.Write("Received: " BackupRequest)
  
  RunCMD(BackupRequest)

  PostActionHandler(BackupRequest)

  loggerWorker.Write("PostAction END")

  launcher.Send("ACK: " BackupRequest)

  loggerWorker.Write("Reply Sent: ACK:" BackupRequest)
