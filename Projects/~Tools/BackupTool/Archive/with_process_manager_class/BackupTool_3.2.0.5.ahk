; TITLE   : BackupTool v3.2.0.5 with ProcessManager
...
ProcessMgr := ProcessManager(MyGui, "SyncBackSE.exe", 500, OnStopCallback)
...
TimeFmt() {
  if (Time.Start>0)
    Time.StartFmt   := FormatTime(Time.Start, "HH:mm:ss")
  if (Time.End>0)
    Time.EndFmt     := FormatTime(Time.End, "HH:mm:ss")
  if (Time.Start>0)
    Time.ElapsedFmt := SecondsFmt(DateDiff("", Time.Start, "Seconds"))
}
...
StartBackup() {
  ...
  WriteStatus("Backup then " SyncBackParams.PostAction " Started at: " Time.StartFmt)
...
OnStopCallback(Reason, Name) {
  ...
  WriteStatus("Backup then "  SyncBackParams.PostAction " " .
              Reason " at: "  Time.EndFmt " " . 
               ", Elapsed: "  Time.ElapsedFmt)
