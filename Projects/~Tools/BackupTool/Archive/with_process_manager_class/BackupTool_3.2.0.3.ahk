; TITLE   : BackupTool v3.2.0.3 with ProcessManager
...
#Include <NamedPipe>
#Include <RunCMD>
;#Include <ShowWaitCursor>
#Include ProcessManagerClass.ahk
...
;@Ahk2Exe-Set ProductVersion, 3.1.0.1
...
proc := ProcessManager(MyGui, "SyncBackSE.exe", 500, OnStopCallback)
...
ButtonCommon_Click(Ctrl, Info){
  ...
  if (timedOut) {
    ...
    StartBackup(SyncBackPostAction) ; Waits for process to exist
...
StartBackup(SyncBackPostAction) {
  Global IsRunning
  global BackupJob
  global BackupRequest
  ...
  BackupJob := "Post-action: " SyncBackPostAction

  WriteStatus(BackupJob)
...
;   global CancelPressed
;   global IsRunning
;   global SyncBackProcessName
;   global StartTime

;   if !IsRunning
;     return

;   ;WinWaitProcess timeoout?

;   if ProcessExist(SyncBackProcessName) {
;     WriteStatus(BackupJob ", Elapsed: " DateDiff("", StartTime, "Seconds"))
;   } else {
;     BackupFinished(BackupJob ", Finished at: " FormatTime(A_Now, "HH:mm:ss") ", Elapsed: " DateDiff("", StartTime, "Seconds"))
...
;     ; kill backup
;     buttonPress:= Msgbox("Kill Backup Job?", "Cancel Pressed", "YesNo Icon?")
...
; BackupFinished(Reason:="Finished") {
;   Global IsRunning
;   Global CancelPressed

;   SetTimer(CheckStatus, 0)
...
;OnStopCallback(*) {
  ...
  WriteStatus(Name " " Reason)
  ;WriteStatus("Reason : ProcessName")
...
ToggleButtons() {
    ...
    if (IsActive) {
        ButtonSelectProfile.Enabled:=true
        ButtonLogs.Enabled:=true
        ButtonNothing.Enabled:=true
        ButtonMonOff.Enabled:=true
        ButtonLogOff.Enabled:=true
        ButtonSleep.Enabled:=true
        ButtonHibernate.Enabled:=true
        ButtonShutdown.Enabled:=true
    } else {
        ButtonSelectProfile.Enabled:=false
        ButtonLogs.Enabled:=false
        ButtonNothing.Enabled:=false
        ButtonMonOff.Enabled:=false
        ButtonLogOff.Enabled:=false
        ButtonSleep.Enabled:=false
        ButtonHibernate.Enabled:=false
