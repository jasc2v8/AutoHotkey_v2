...
ButtonCommon_Click(Ctrl, Info){
  ...
  if (timedOut) {
    ...
    ; Start BackupControlTask
    if (!StartTask(TASK_PATH)) {
      MsgBox "Timeout waiting for Task to start", "Timeout"
      return
    }
    ...
    WriteLog("Command to Server: " command)

    response := myPipe.Send(command)
    ...
    ; The Task will end after one run

    ;response := PipeHelper.SendRequest("TERMINATE")
...
StartTask(TASK_PATH) {
  ...
  ; Allow time for task to start and create pipe
  Sleep 500

  ; Check if task was started
  ; 0=timeout, PID=no timeout
  return ProcessWait("BackupControlTask.exe", 1)
