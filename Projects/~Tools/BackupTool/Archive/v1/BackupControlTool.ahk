...
; Uses a Shared Memory to communicate with AhkRunCmdService who actually runs Syncback
...
; #region Shared Memory access

name := "AhkRunCmdServiceSharedMemory"
size := 1024
...
global hEventCLIENT_DATA_READY := mem.CreateEvent(CLIENT_DATA_READY)
global hEventSERVER_DATA_READY := mem.CreateEvent(SERVER_DATA_READY)

; #region Is Service Ready?
...
ButtonCommon_Click(Ctrl, Info){
 ...
 WriteStatus("Ready.")
  ...
  if (timeout) {
    ...
    BackupProfile := '"' Trim(TextProfile.Text) '"'

    ; success = output, error = ""
    ;r := RunCMD.Array(SyncBackPath, BackupParameter, Profile)

    command := mem.ConvertToCSV(SyncBackPath, BackupParameter, BackupProfile)

    r := RunCMD.CSV(command)
...
IsServiceReady() {
  ...
  ; Check if AhkRunCmdService is running
  mem.Write("STATUS")

  ; Signal the event so the service knows data is ready
  mem.SetEvent(CLIENT_DATA_READY)

  ; Wait 5 seconds for the service to send a response
  serviceAvailable := mem.WaitEvent(SERVER_DATA_READY, 5000)

  ; reset the event for the next time
