...
; REQUIRES A PASSWORD TO CHANGE TARGET - NOT WORTH THE EFFORT, JUST CHANGE IN TASK SCHEDULER
...
class TaskGui
{
    ...
    static Show()
    {
        ...
        this.MyMenu := Menu()
        this.MyMenu.Add("Copy", _CopyRowText)
        this.MyMenu.Add("Refresh", (*) => Reload())

        ; --- 3. Attach ContextMenu Event to ListView ---
        ; This function runs when you right-click the ListView
        this.lv.OnEvent("ContextMenu", _ShowLVMenu)

        this.gui.Show()
        this.Refresh()

            _CopyRowText(*) {
                RowNumber := this.lv.GetNext(0) ; Get the currently selected row
                
                if (RowNumber = 0)
                    return ; Separate lines
                    
                ; Get text from the first column (Column 1)
                RowData:= ""
                Loop 5 {
                    RowData .= this.lv.GetText(RowNumber, A_Index) ","
                }
                ...
                ;ToolTip("Copied: " . RowData)
                ToolTip("Copied RowData to Clipboard.")
                SetTimer(() => ToolTip(), -2000) ; Clear tooltip after 2 seconds
            }

            _ShowLVMenu(GuiCtrl, Item, IsRightClick, X, Y) {

                ; Item is the row number that was right-clicked (0 if clicked on empty space)
                if (Item = 0)
                    return ; Following rule: separate lines for if and return
                    
                ; Select the row that was right-clicked (optional, but improves UX)
                this.lv.Modify(Item, "Select Focus")
                
                ; Display the menu at the mouse cursor coordinates
    ...
    static EditTarget(*) {
        ...
        RowNumber := this.lv.GetNext(0)

        target := this.lv.GetText(RowNumber, 5)

        response := InputBox("Task:`n" task "`n`nTarget:`n" target "`n`nEnter New Target:", "Edit Target",,"Enter New Target...")

        password := InputBox("User Password:","Enter Password", "Password")

        if (password.Value="")
            return

        if (response.Result = "OK")
            this.RunAsAdmin("/target", task, response.Value, password.Value)
    }
    ...
    static AdminAction(args)
    {
        ...
        switch action
        {
            case "/target":
                ; msg := ""
                ; Loop args.Length {
                ;     msg .= A_Index ": " args[A_Index] ", "
                ; }
                ; MsgBox msg, "Args"
                this.ExecSilent('schtasks /Change /TN "' args[3] '" /TR "' args[4] '" /P "' args[5] '"')
