; TITLE  :  RegEditUser v4.8.5.1
; SOURCE :  Gemini and jasc2v8
; LICENSE:  The Unlicense, see https://unlicense.org
; PURPOSE:  Registry User Edit Utility (HKCU)
; USAGE  :
; NOTES  :
...
MyGui := Gui("+Resize", "Registry Edit HKCU v4.8.5")
...
IL_Add(IL_Small, "HICON:" . IconKey)  ; Icon 1: Folder/Key
IL_Add(IL_Small, "HICON:" . IconDoc)  ; Icon 2: Value/Doc
...
{
    LastPath := reg.Read("LastKey")
    if (LastPath = "" || LastPath = "HKEY_CURRENT_USER")
        return

    ; Strip root to navigate children
    ...
    {
        ; Trigger subkey loading if necessary (Lazy Loading workaround)
        FirstChild := TreeObj.GetChild(CurrentID)
        if (FirstChild != 0 && TreeObj.GetText(FirstChild) == "...")
        {
            TreeObj.Delete(FirstChild)
            ParentPath := GetFullPath(TreeObj, CurrentID)
            AddSubKeys(ParentPath, CurrentID, TreeObj, false)
        }
        
        ; Locate child matching the path segment
...
UpdateView(Source, ItemID)
{
    ...
    reg.Write("LastKey", FullPath)
    
    if (FullPath = "HKEY_CURRENT_USER")
    {
        LV.Delete()
        return
    }
...
CreateValue(Type)
{
    ...
    if (CurrentPath = "HKEY_CURRENT_USER")
        return
...
DeleteSelectedKey(*)
{
    ...
    if (TargetPath = "HKEY_CURRENT_USER")
        return
