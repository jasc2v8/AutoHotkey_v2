...
ButtonMerge_Click(Ctrl, Info) {
    ...
    ; TODO only read script once into MainScript

    MainScriptMap := ScanScriptToMap(Trim(ScriptEdit.Text))

    ;ObjView("MainScriptMap", MainScriptMap )



    IncludeFileArray := GetIncludeFileArray(Trim(ScriptEdit.Text))
    ...
    MergedScript := MergeScript(MainScriptMap, IncludeMap)
...
MergeScript(MainScriptMap, IncludeMap) {
...
GetIncludeFileArray(ScriptFilePath) {
...
GetIncludeMap(IncludeFileArray) {
    ...
    for ScriptFilePath in IncludeFileArray {
        ...
        if !Str.IsEmpty(ScriptFilePath) {
            ;script := FileRead(includeFile)

            ;myMap := ScanScriptToMap(ScriptFilePath)
            ;ObjView("GetIncludeMap", myMap)

           combinedMap := _CombineMaps(combinedMap, ScanScriptToMap(ScriptFilePath))
