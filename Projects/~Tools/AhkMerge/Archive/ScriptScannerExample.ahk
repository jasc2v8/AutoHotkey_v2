 ...
 * @returns {Array} - An array of strings ["Type,Name,Line,Length,Path", ...]
 */
ScanScriptToCSV(ScriptText, BaseDir := A_ScriptDir, CurrentFilePath := A_ScriptFullPath) {
    if (ScriptText = "")
        return
...
;ExampleCode:= FileRead("D:\Software\DEV\Work\AHK2\Projects\~Tools\AhkMerge\BackupToolTest.ahk")
;ExampleCode:= FileRead("C:\Users\Jim\Documents\AutoHotkey\Lib\String_Functions.ahk")
ExampleCode:= FileRead("C:\Users\Jim\Documents\AutoHotkey\Lib\IniFile.ahk")

; Get the Array of CSV strings
CSVArray := ScanScriptToCSV(ExampleCode, A_ScriptDir, A_ScriptFullPath)

; Build display string for the MsgBox
Output := "Type,Name,Line,Length,Path`n"
Output .= "------------------------------------------------------------`n"
for Row in CSVArray {
    Output .= Row . "`n"
}
