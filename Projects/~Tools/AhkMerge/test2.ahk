...
MyCheckBoxExcludeUnusedClassesAndFunctions :=
    myGui.AddCheckbox("xm w350", "Exclude Unused Classes and Functions")
...
MyCheckBoxExcludeHeaders.OnEvent("Click", Button_Change)
...
class MergeScript {
    ...
    static Includes(ScriptFile) {
        ...
        ;DEBUG
        ; split := this.IncludedFiles.Split(",")
        ; for file in split {
        ;     ;Debug.MBox "File: " file, "DEBUG", "icon?"
        ;     FunctionsCSV := ScanLibScript(file)
        ;     Debug.ListVar FunctionsCSV
        ;     Debug.FileWriteLine(FunctionsCSV, "FunctionsCSV.txt", True)
        ;     Debug.MBox "Check FunctionsCSV.txt", "DEBUG", "icon?"
        ; }
    ...
    static _MergeIncludes(ScriptFile, IncludedFiles) {
        ...
        for includeFile in split {
            ...
            if MyCheckBoxExcludeUnusedClassesAndFunctions.Value {
                ...
                buffer .= this._ReadInclude(ScriptFile)
             }
    ...
    static _ReadInclude(ScriptFile) {
        ...
        Loop ScriptTextArray.Length {
            ...
            if (line.StartsWith("*/") AND MyCheckBoxExcludeComments.Value) {
                InCommentBlock := false
                continue
            }
            ...
            if line.StartsWith("#Requires AutoHotkey")
                line := ";" line "`n"
    ...
    static _ReadFunction(ScriptFile, IncludeLine) {
        ...
        ; if function.Contains("String2")
        ;     Debug.MBox(function ", " this.MainScriptText.Contains(function), "ReadFunction")

        if NOT this.MainScriptText.Contains(function)
            return
        ...
        ; if function.Contains("String2")
        ;     Debug.MBox("ScriptTextArray.Length: "  ScriptTextArray.Length , "ReadFunction")
    ...
    ; static _IfContains(ScriptFile, FunctionsCSV) {
    ;     split := FunctionsCSV.Split(",")
    ;     for function in split {
    ;         if ScriptFile.Contains(function)
    ;             return true
    ;     }
    ;     return false
    ; }
