...
ButtonMerge_Click(Ctrl, Info) {
    ...
    ;IncludeFileArray := GetIncludeFileArray(MainScriptPath)
    ...
    ;MergedScript := MergeScript(MainScript, IncludeScript)
...
SaveMergedScript(MainScriptPath, IncludeFileMap) {
    ...
    for filePath, v in IncludeFileMap {
        ...
        script := Str.Replace(script, "#Include" , ";#Include")
        script := Str.Replace(script, "#Requires", ";#Requires")
        MergedIncludes .= "`n" . script
    }
    ...
    for filePath, v in IncludeFileMap
        ;MergedScript := AddHeader(MergedScript, ";  " Str.RTrim(filePath, ",") "`n")
        MergedScript := AddHeader(MergedScript, ";  " filePath "`n")
...
ScanScriptToMap(ScriptText, ScriptFullPath) {
    ...
    ; Removes all comments (block and inline) and extra whitespace.
    ;ScriptText := CleanScript(ScriptText)

;MsgBox ScriptText, "ScriptText"
    ...
    Loop scriptArray.Length {
        ...
        if RegExMatch(Line, "i)^class\s+(?P<Name>[a-zA-Z0-9_]+)", &Match) {
            ...
            itemLine := A_Index
             ...
             MsgBox ScriptFullPath "`n`n" line "`n`n" itemtype "`n`n" itemName "`n`n" itemLine "`n`n" 0, "DEBUG 1 " ItemType
        ...
        if !Str.IsEmpty(itemType) {
            ...
            if itemType = "Class"
                MsgBox ScriptFullPath "`n`n" line "`n`n" itemtype "`n`n" itemName "`n`n" itemLine "`n`n" blockCount, "DEBUG " ItemType
        }
    }
    return objMap
}
...
CleanScript(ScriptContent) {
    ...
    ; \R  = Any newline sequence (CRLF, LF, etc.)
    ;return RegExReplace(ScriptContent, "m)^\s*\R", "")
...
;MsgBox "ScanInclude:`n`n" IncludeFilePath

objMap := ScanScriptToMap(script, IncludeFilePath)

ObjView("ScanIncludesToMap: " IncludeFilePath, objMap )

            ;combinedMap := _CombineMaps(combinedMap, ScanScriptToMap(script, IncludeFilePath))
            combinedMap := _CombineMaps(combinedMap, objMap)

ObjView("ScanIncludesToMap - combinedMap", combinedMap )
