; TITLE  :  AhkProcessManager v1.1 (added WM_ACTIVATE)
; SOURCE :  Gemini
...
MyGui := Gui("+Resize", "AHK Process Inspector " . AdminStatus)
...
; 2. Create ListView (Multi-select is enabled by default)
...
ProcessMenu.Add("Copy Row Info", Menu_CopyRow) ; New Option
ProcessMenu.Add() 
ProcessMenu.Add("Kill Selected Process(es)", Menu_KillProcess) ; Updated Label
...
WM_ACTIVATE(wParam, lParam, msg, hwnd) {
    ; Check if the message is for our GUI's handle
    if (hwnd = MyGui.Hwnd) {
        UpdateList()
    }
}
...
Menu_Properties(*) {
    if (Row := LV.GetNext()) {
        ...
        try {
            ...
            MsgBox("Error accessing process properties.")
        }
    }
}
...
Menu_CopyRow(*) {
    ...
    while (Row := LV.GetNext(Row)) {
        ...
        PID   := LV.GetText(Row, 3)
        Class := LV.GetText(Row, 4)
        SelectedData .= "Title: " Title "`nName: " Name "`nPID: " PID "`nClass: " Class "`n`n"
    }
    if SelectedData {
        A_Clipboard := RTrim(SelectedData, "`n")
        MsgBox("Information copied to clipboard.", "Success", "Iconi T2")
    }
}
...
Menu_KillProcess(*) {
    ...
    msg := (count = 1) ? "Kill the selected process?" : "Kill all " count " selected processes?"
    
    if (MsgBox(msg, "Confirm Termination", 4 + 256) == "Yes") {
        Loop SelectedRows.Length {
            ; We process in reverse to avoid index shifting if we were deleting rows, 
            ; though here we just refresh at the end.
            CurrentRow := SelectedRows[SelectedRows.Length - A_Index + 1]
            PID := LV.GetText(CurrentRow, 3)
            try {
                ProcessClose(PID)
            } catch {
                MsgBox("Failed to terminate PID: " PID)
            }
        }
        UpdateList()
    }
