...
; --- Configuration ---
;SourceFolder := A_ScriptDir '\MyFilesToBackup' ; The folder you want to copy (e.g., in the same directory as the script)
SourceFolder := "D:\Software\DEV\Work\AHK2\Projects\BackupControlTool\"
BackupRoot := A_ScriptDir '\Backups\'        ; The root directory where backups will be placed
Timestamp := FormatTime(, "yyyyMMdd_HHmmss") ; Get a clean timestamp (e.g., 20251104_075530)
DestinationFolder := BackupRoot . "Backup_" . Timestamp

;MsgBox SourceFolder "`n`n" DestinationFolder

SplitPath(DestinationFolder, &Name, &Dir)
if (!DirExist(Dir))
   DirCreate(Dir)
...
; --- Execution ---
If FolderBackup(SourceFolder, DestinationFolder)
    MsgBox("Backup successful!`n`nSource:`n`n" . SourceFolder . "`n`nDestination:`n`n" . DestinationFolder, "Backup Complete", 0x40)
Else
 ...
 */
FolderBackup(Source, Destination, Overwrite := false)
{
    try
    {
